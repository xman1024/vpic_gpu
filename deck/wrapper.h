...
#define begin_diagnostics void vpic_simulation::user_diagnostics(void)
...
#define begin_field_injection void vpic_simulation::user_field_injection(void)

#define begin_particle_collisions \
    void vpic_simulation::user_particle_collisions(void)

#define repeat(count) \
    for (int64_t _remain = (int64_t)(count); _remain; _remain--)
...
#define set_point_region_material(rgn, rmat)                         \
    do {                                                             \
        ...
        for (int _k = 0; _k < _nz + 2; _k++) {                       \
            const double _zn = _z0 + _dz * (_k - 1),                 \
                         _zc = _z0 + _dz * (_k - 0.5);               \
            for (int _j = 0; _j < _ny + 2; _j++) {                   \
                const double _yn = _y0 + _dy * (_j - 1),             \
                             _yc = _y0 + _dy * (_j - 0.5);           \
                field_t* _f      = &field(0, _j, _k);                \
                for (int _i = 0; _i < _nx + 2; _i++) {               \
                    const double _xn = _x0 + _dx * (_i - 1),         \
                                 _xc = _x0 + _dx * (_i - 0.5);       \
                    double x, y, z;                                  \
                    x = _xn;                                         \
                    y = _yn;                                         \
...
#define set_point_region_bc(rgn, ipbc, epbc)                                   \
    do {                                                                       \
        ...
        for (int _k = 1; _k < _nz + 1; _k++) {                                 \
            const double _zn = _z0 + _dz * (_k - 1), _zh = _z0 + _dz * _k;     \
            for (int _j = 1; _j < _ny + 1; _j++) {                             \
                const double _yn = _y0 + _dy * (_j - 1), _yh = _y0 + _dy * _j; \
                int64_t* _n = _n0 + 6 * voxel(1, _j, _k);                      \
                for (int _i = 1; _i < _nx + 1; _i++) {                         \
                    const double _xn = _x0 + _dx * (_i - 1),                   \
                                 _xh = _x0 + _dx * _i;                         \
                    double x, y, z;                                            \
                    int _r000, _r100, _r010, _r110, _r001, _r101, _r011,       \
                        _r111;                                                 \
                    x     = _xn;                                               \
                    y     = _yn;                                               \
                    z     = _zn;                                               \
                    ...
                    _r111 = (rgn);                                             \
                    if (_epbc) {                                               \
                        if (_r000 || _r010 || _r001 || _r011)                  \
                            _n[0] = _epbc;                                     \
                        if (_r000 || _r001 || _r100 || _r101)                  \
                            _n[1] = _epbc;                                     \
                        if (_r000 || _r100 || _r010 || _r110)                  \
                            _n[2] = _epbc;                                     \
                        if (_r100 || _r110 || _r101 || _r111)                  \
                            _n[3] = _epbc;                                     \
                        if (_r010 || _r011 || _r110 || _r111)                  \
                            _n[4] = _epbc;                                     \
                        if (_r001 || _r101 || _r011 || _r111)                  \
                            _n[5] = _epbc;                                     \
                    }                                                          \
                    if (_ipbc) {                                               \
                        if (_r000 && _r010 && _r001 && _r011)                  \
                            _n[0] = _ipbc if (_r000 && _r001 && _r100 &&       \
                                              _r101) _n[1] =                   \
                                _ipbc if (_r000 && _r100 && _r010 && _r110)    \
                                    _n[2] = _ipbc if (_r100 && _r110 &&        \
                                                      _r101 && _r111) _n[3] =  \
                                        _ipbc if (_r010 && _r011 && _r110 &&   \
                                                  _r111) _n[4] =               \
                                            _ipbc if (_r001 && _r101 &&        \
                                                      _r011 && _r111) _n[5] =  \
                                                _ipbc                          \
                    }                                                          \
                    _n += 6;                                                   \
...
#define set_point_region_field(rgn, eqn_ex, eqn_ey, eqn_ez, eqn_bx, eqn_by, \
                               eqn_bz)                                      \
    do {                                                                    \
        const double _x0 = grid->x0, _y0 = grid->y0, _z0 = grid->z0;        \
        const double _dx = grid->dx, _dy = grid->dy, _dz = grid->dz;        \
...
#define set_region_material(rgn, vmat, smat)                             \
    do {                                                                 \
            ...
            for (int _j = 0; _j < _ny + 2; _j++) {                       \
                ...
                for (int _i = 0; _i < _nx + 2; _i++) {                   \
                    ...
                    if (_smat != -1) {                                   \
                        ...
                        if (_rccc || _rccl)                              \
                            _f->fmatz = _smat;                           \
                        if (_rccc || _rlcc || _rclc || _rllc || _rccl || \
                            _rlcl || _rcll || _rlll)                     \
                            _f->nmat = _smat;                            \
                    }                                                    \
                    if (_vmat != -1) {                                   \
...
#define set_region_bc(rgn, vpbc, ipbc, epbc)                              \
    do {                                                                  \
        const int64_t _vpbc = get_particle_bc_id((particle_bc_t*)(vpbc)); \
        const int64_t _ipbc = get_particle_bc_id((particle_bc_t*)(ipbc)); \
        const int64_t _epbc = get_particle_bc_id((particle_bc_t*)(epbc)); \
        if (!_vpbc && !_ipbc && !_epbc)                                   \
            break;                                                        \
        const double _x0 = grid->x0, _y0 = grid->y0, _z0 = grid->z0;      \
        const double _dx = grid->dx, _dy = grid->dy, _dz = grid->dz;      \
        const int _nx = grid->nx, _ny = grid->ny, _nz = grid->nz;         \
...
#define define_volume_emitter(e, rgn)                                \
    do {                                                             \
        ...
        for (int _k = 1; _k < _nz + 1; _k++) {                       \
            ...
            for (int _j = 1; _j < _ny + 1; _j++) {                   \
                ...
                for (int _i = 1; _i < _nx + 1; _i++) {               \
                    ...
                    z = _zc;                                         \
                    if ((rgn))                                       \
                        _c[_nc++] = COMPONENT_ID(voxel(_i, _j, _k),  \
                                                 BOUNDARY(0, 0, 0)); \
                }                                                    \
            }                                                        \
        }                                                            \
...
#define define_surface_emitter(e, rgn)                                \
    do {                                                              \
        ...
        for (int _k = 1; _k < _nz + 1; _k++) {                        \
            ...
            for (int _j = 1; _j < _ny + 1; _j++) {                    \
                ...
                for (int _i = 1; _i < _nx + 1; _i++) {                \
                    ...
                    _r5 = (rgn);                                      \
                    if (!_rc && _r0)                                  \
                        _c[_nf++] = COMPONENT_ID(voxel(_i, _j, _k),   \
                                                 BOUNDARY(-1, 0, 0)); \
                    if (!_rc && _r1)                                  \
                        _c[_nf++] = COMPONENT_ID(voxel(_i, _j, _k),   \
                                                 BOUNDARY(0, -1, 0)); \
...
#define set_region_field(rgn, eqn_ex, eqn_ey, eqn_ez, eqn_bx, eqn_by, eqn_bz) \
    do {                                                                      \
        ...
        const int _nx = grid->nx, _ny = grid->ny, _nz = grid->nz;             \
        for (int _k = 0; _k < _nz + 2; _k++) {                                \
            const double _zl = _z0 + _dz * (_k - 1.5), _ze = _z0 + _dz * _k,  \
                         _zc = _z0 + _dz * (_k - 0.5);                        \
            for (int _j = 0; _j < _ny + 2; _j++) {                            \
                const double _yl = _y0 + _dy * (_j - 1.5),                    \
                             _ye = _y0 + _dy * _j,                            \
