...
void vpic_simulation::user_initialization(int num_cmdline_arguments,
    ...
    for (int n = 0; n < nstep; n++) {
        ...
        for (int n = 0; n < accumulator_array->n_pipeline + 1; n++) {
            accumulator_t* a =
                accumulator_array->a + (n * accumulator_array2->stride);
            accumulator_t* a2 =
                accumulator_array2->a + (n * accumulator_array2->stride);
            for (int i = 0; i < grid->nv; i++) {
                if ((a[i].jx[0] != a2[i].jx[0]) ||
                    (a[i].jx[1] != a2[i].jx[1]) ||
                    (a[i].jx[2] != a2[i].jx[2]) ||
                    (a[i].jx[3] != a2[i].jx[3]) ||
                    (a[i].jy[0] != a2[i].jy[0]) ||
...
TEST_CASE(
    "test that calling the push with array syntax matches the result of the "
    "old method",
    "[particle_push]") {
    int pargc    = 0;
    char str[]   = "bin/vpic";
