...
/**
 * @brief Helper function to write collective errors to file for further
 * analysis
 *
 * @param errs The vector of all errors
 * @param field_per_line The number of values to write per file line
 */
void write_error_ouput(std::vector<double> errs,
                       int field_per_line,
                       std::string err_file_base_name) {
    int counter = 0;
    std::ofstream outputFile(err_file_base_name);

    for (auto e : errs) {
        counter++;
        outputFile << counter << " " << e * 100.0
                   << " ";  // Convert to percent and dump
...
/**
 * @brief Function to compare errors to a given tolerance, and decide if it's
 * within range
 *
 * @param A The first value to compare
 * @param B The second value to compare
 ...
 *
 * @return A pair containing true/false if it's within tolerance, and the
 * calculated error
 */
std::pair<bool, double> compare_error(double A,
                                      double B,
...
bool compare_energies(
    ...
    const unsigned short field_mask =
        0b1111111111111111,  /// short has 16 bytes, assume all are true
    const FIELD_ENUM field_enum =
        FIELD_ENUM::Individual,     /// short has 16 bytes, assume all are true
    const int write_err_ouput = 0,  // If the run should dump the errors to disk
    const std::string err_file_base_name =
        "err.out",  // File name to write errors to
    const int num_lines_to_skip =
        0  // Most energy files have 3 lines of padding
) {
    // TODO: I could easily have a policy here based on the type of the
    // field_mask
    ...
    try {
        ...
        } else  // Performan test
        {
            ...
            while (getline(f1, line1)) {
                ...
                while (getline(linestream1, item1, ' ')) {
                    ...
                    // std::cout << "Setting write_this_err_ouput tp " <<
                    // write_this_err_ouput << std::endl;

                    getline(linestream2, item2, ' ');
                    total_line_token_count++;

                    // Use this field
                    // std::cout << "this_line " << this_line_token_count << "
                    // mask " << field_mask << std::endl;

                    // Take the value one, and shift it to generate the mask to
                    // compare
                    unsigned short this_line_token_mask =
                        1 << (total_line_token_count -
                              1);  // Set correct highest bit on
                    // this_line_token_mask |= this_line_token_mask-1; // Set
                    // lower bits on

                    // If this field is within our requested mask, use it
                    if (this_line_token_mask & field_mask) {
                        used_line_token_count++;
                        // std::cout << "Parsing field " <<
                        // used_line_token_count << " val " << item1 <<
                        // std::endl;
                        ...
                        if ((field_enum ==
                             FIELD_ENUM::Sum) &&     // Need to aggregate
                            ...
                            // std::cout << "sum a " << sum_A << " += " << A <<
                            // std::endl; std::cout << "sum b " << sum_B << " +=
                            // " << B << std::endl;
                            ...
                            if (agg_count == agg_total) {  // final_aggregation
                                returned_err = compare_error(
                                    sum_A, sum_B, relative_tolerance);
                                write_this_err_ouput = true;
                            }
                        } else  // We can just compare this val
                        {
                            returned_err =
                                compare_error(A, B, relative_tolerance);
                        }
                        ...
                        // std::cout << "Skipping field " <<
                        // this_line_token_mask << " val " << item1 <<
                        // std::endl;
                    }
                }
        ...
        std::cout << "Max found err was " << max_err * 100 << "% on line "
                  << max_err_line << " (Threshold: " << relative_tolerance * 100
                  << "%)" << std::endl;
        ...
        return match;
                 ...
                 exc)  // Catching all is bad form, but OK for now..
    {
