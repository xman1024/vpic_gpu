...
begin_initialization {
    ...
    // Then the initial non-zero fields need to be loaded at time level 0 and
    // the particles (position and momentum both) need to be loaded at time
    // level 0.
    ...
    // Physics parameters
    double mi_me =
        1836;  // 25; //atof(cmdline_argument[1]); // Ion mass / electron mass
    double vthe = 0.25 / sqrt(2.0);  // 0.0424264068711;       //0.424264068711;
                                     // // Electron thermal velocity
    double vthi = 0.25 / sqrt(2.0);  // 0.0424264068711;       //0.424264068711;
                                     // // Ion thermal velocity
    double vthex =
        0.05 / sqrt(2.0);  // 0.0141421356237;      // 0.141421356237;      //
                           // Electron thermal velocity in x-direction.
    double vthix =
        0.05 / sqrt(2.0);  // 0.0141421356237;      // 0.141421356237;Ion
    ...
    double topology_y = 1;
    double topology_z =
        1;  // For load balance, best to keep "1" or "2" for Harris sheet
    double Lx = 2.09439510239320;  // 4.62*de; //6.7*de; //10.0*de;  // How big
                                   // should the box be in the x direction
    double Ly =
        1;  // 0.0721875*de;  // How big should the box be in the y direction
    double Lz =
        1;  // 0.0721875*de;  // How big should the box be in the z direction
    double nx =
        16;  // 64; //64; //32;    // Global resolution in the x direction
    double ny = 1;  // Global resolution in the y direction
    double nz = 1;  // 32;     // Global resolution in the z direction
    double nppc =
        200;  // 800; //200; //2048; //1024; //128;    // Average number of
              // macro particles per cell (both species combined!)
    double cfl_req =
        0.99f;  // 0.99;  // How close to Courant should we try to run
    double wpedt_max = 0.36;  // How big a timestep is allowed if Courant is not
                              // too restrictive
    ...
    // exit(1);
    if (wpe * dt > wpedt_max)
        dt = wpedt_max / wpe;  // Override time step if plasma frequency limited
    ...
    // species_t * ion      = define_species( "ion",       ec,
    // mi, 1.5*Ni/nproc(), -1, 40, 1 ); species_t * electron = define_species(
    // "electron", -ec, me, 1.5*Ne/nproc(), -1, 20, 1 );
    // species_t *electron =
    // define_species("electron",-ec,me,2.4*Ne/nproc(),-1,25,0); species_t *ion
    // = define_species("ion",      ec,mi,2.4*Ne/nproc(),-1,25,0);

    species_t* electron =
        define_species("electron", -ec, me, 2.4 * Ne / nproc(), -1, 0,
                       0);  // turn off sorting (GY)
    species_t* ion =
        define_species("ion", ec, mi, 2.4 * Ne / nproc(), -1, 0, 0);  //(GY)
    ...
    // field
    //                   0, -sn*b0*tanh(x/L), cs*b0*tanh(x/L) ); // Magnetic
    //                   field
    ...
    repeat(Ne / nproc()) {
        ...
        inject_particle(electron, x, y, z, n1, n2, n3, we, 0, 0);

        n1 = normal(rng(0), 0, vthix);
        n2 = normal(rng(0), 0, vthi);
        n3 = normal(rng(0), 0, vthi);

        inject_particle(ion, x, y, z, n1, n2, n3, wi, 0, 0);
    }
...
TEST_CASE("Check if Weibel gives correct energy (within tol)", "[energy]") {
    ...
    SECTION("e_field") {
        ...
        REQUIRE(test_utils::compare_energies(
            energy_file_name, energy_gold_file_name, 0.3, e_mask,
            test_utils::FIELD_ENUM::Sum, 1, "Weibel.e.out"));
    }

    SECTION("b_field") {
        // Test the sum of the b_field
        REQUIRE(test_utils::compare_energies(
            energy_file_name, energy_gold_file_name, 0.03, b_mask,
            test_utils::FIELD_ENUM::Sum, 1, "Weibel.b.out"));
    }

    SECTION("particle_energy") {
        // Test particle energies individually
        REQUIRE(test_utils::compare_energies(
            energy_file_name, energy_gold_file_name, 0.01, particle_mask,
            test_utils::FIELD_ENUM::Sum, 1, "Weibel.p.out"));
    }
}
