...
void advance_p2_pipeline_scalar(advance_p_pipeline_args_t* args,
                                int pipeline_rank,
                                int n_pipeline) {
...
void advance_p2(/**/ species_t* RESTRICT sp,
    ...
    for (rank = 0; rank <= N_PIPELINE; rank++) {
        if (args->seg[rank].n_ignored)
            WARNING(("Pipeline %i ran out of storage for %i movers", rank,
