...
#define STRIP_CMDLINE(what, T, convert)                                       \
    T strip_cmdline_##what(int* pargc, char*** pargv, const char* key,        \
                           T val) {                                           \
        int i, n = 0;                                                         \
        for (i = 0; i < (*pargc); i++)                                        \
            if (strcmp((*pargv)[i], key))                                     \
                (*pargv)[n++] = (*pargv)[i];                                  \
            else if ((++i) < (*pargc))                                        \
                val = convert((*pargv)[i]);                                   \
...
void detect_old_style_arguments(int* pargc, char*** pargv) {
    // FIXME: This could also warn for unknown options being passed (such as
    // typos)
    int i;
    ...
    for (i = 0; i < (*pargc); i++) {
        ...
        for (j = 0; j < num_prefix_keys; j++) {
            ...
            if (string_starts_with(arg, prefix_keys[j])) {
                ...
                sprintf(output_message,
                        "Aborting. Single dashed flag %1$s is invalid (needs "
                        "'-%1$s').",
                        prefix_keys[j]);
        ...
        for (j = 0; j < num_match_keys; j++) {
            if (string_matches(arg, match_keys[j])) {
                // TODO: This could be tightened up more, as it disallows the
                // use of a file called 'restart'
                char output_message[64];
                sprintf(output_message, "Old Argument Syntax Detected: %s",
                        match_keys[j]);
        ...
        if (string_contains(arg, "=")) {
            ...
            for (j = 0; j < NUM_WARN_REPEAT; j++) {
                WARNING(
                    ("Arguments contains '=', is this intentional? (use a "
                     "space)"));
            }
        }
    }
...
void util_malloc(const char* err, void* mem_ref, size_t n) {
    char* mem;

    // If no err given, use a default error
    if (!err)
        err = "malloc failed (n=%lu)";
...
void util_malloc_aligned(const char* err, void* mem_ref, size_t n, size_t a) {
