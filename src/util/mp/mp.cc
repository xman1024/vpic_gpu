...
void halt_mp(void) {
    MPWrapper::instance().halt_mp();
}

void mp_abort(int reason) {
    MPWrapper::instance().mp_abort(reason);
}

void mp_barrier(void) {
    MPWrapper::instance().mp_barrier();
}
...
mp_t* new_mp(int n_port) {
    return MPWrapper::instance().new_mp(n_port);
}

void delete_mp(mp_t* mp) {
    MPWrapper::instance().delete_mp(mp);
}
