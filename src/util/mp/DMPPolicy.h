...
collective_t* restore_collective(void) {
    ...
    if (size != world_size)
        ERROR(
            ("The number of nodes that made this checkpt (%i) is different "
             "from the number of nodes currently (%i)",
             size, world_size));
    if (rank != world_rank)
        ERROR(
            ("This node (%i) is reading a checkpoint previously written by "
             "a different node (%i).",
             rank, world_rank));
    return world;
}
...
#define TRAP(x)                                   \
    ...
    inline void mp_abort(int reason) { MPI_Abort(world->comm, reason); }
    ...
    inline void mp_allgather_i64(int64_t* sbuf, int64_t* rbuf, int n) {
        if (!sbuf || !rbuf || n < 1)
            ERROR(("Bad args"));
        TRAP(MPI_Allgather(sbuf, n, MPI_LONG_LONG, rbuf, n, MPI_LONG_LONG,
                           world->comm));
    }
    ...
    inline void mp_begin_recv(mp_t* mp, int port, int sz, int src, int tag) {
        if (!mp || port < 0 || port >= mp->n_port || sz < 1 ||
            ...
            ERROR(("Bad args"));
        mp->rreq_sz[port] = sz;
        TRAP(MPI_Irecv(mp->rbuf[port], sz, MPI_BYTE, src, tag, world->comm,
                       &mp->rreq[port]));
    }
    ...
    inline void mp_begin_send(mp_t* mp, int port, int sz, int dst, int tag) {
        if (!mp || port < 0 || port >= mp->n_port || dst < 0 ||
            ...
            ERROR(("Bad args"));
        mp->sreq_sz[port] = sz;
        TRAP(MPI_Issend(mp->sbuf[port], sz, MPI_BYTE, dst, tag, world->comm,
                        &mp->sreq[port]));
    }
