...
#define BEGIN_TURNSTILE(n_turnstile)                          \
    do {                                                      \
...
mp_t* new_mp(int n_port);

void delete_mp(mp_t* mp);

void* ALIGNED(128) mp_recv_buffer(mp_t* mp, int port);

void* ALIGNED(128) mp_send_buffer(mp_t* mp, int port);

void mp_size_recv_buffer(mp_t* mp, int port, int size);

void mp_size_send_buffer(mp_t* mp, int port, int size);

// FIXME: MP REALLY SHOULD HANDLE THE MESSAGE TAGGING
void mp_begin_recv(mp_t* mp, int port, int sz, int src, int tag);

void mp_begin_send(mp_t* mp, int port, int sz, int dst, int tag);
