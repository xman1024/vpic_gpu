...
static void dump_node(registry_t* node) {
    Dl_info dli[1];
    if (node->checkpt_func)
        dladdr((void*)(size_t)node->checkpt_func, dli);
    else
        dli->dli_sname = NULL, dli->dli_fname = NULL;
    log_printf(
        "Object %lu:\n"
        "\taddr=%lx\n"
        "\tcheckpt_func=%lx (%s from %s)\n",
        (unsigned long)node->id, (unsigned long)node->obj,
        (unsigned long)node->checkpt_func,
        !dli->dli_sname ? "(null)" : dli->dli_sname,
        !dli->dli_fname ? "(null)" : dli->dli_fname);
    if (node->restore_func)
        dladdr((void*)(size_t)node->restore_func, dli);
    else
        dli->dli_sname = NULL, dli->dli_fname = NULL;
    log_printf("\trestore_func=%lx (%s from %s)\n",
               ...
               !dli->dli_sname ? "(null)" : dli->dli_sname,
               !dli->dli_fname ? "(null)" : dli->dli_fname);
    if (node->reanimate_func)
        dladdr((void*)(size_t)node->reanimate_func, dli);
    else
        dli->dli_sname = NULL, dli->dli_fname = NULL;
...
void boot_checkpt(int* pargc, char*** pargv) {
    ...
    if (booted)
        ERROR(("checkpt service already booted."));
...
void halt_checkpt(void) {
    ...
    if (!booted)
        ERROR(("checkpt service not booted."));
    if (registry) {
        dump_registry();
        ERROR(("halt called with some objects still registered"));
    }
    if (checkpt)
        ERROR(("currently writing a checkpt"));
    if (restore)
        ERROR(("currently reading a checkpt"));
...
size_t object_id(const void* obj) {
    registry_t* node;
...
void unregister_object(void* obj) {
    ...
    for (prev = NULL, node = registry; node; prev = node, node = node->next)
        if (node->obj == obj)
            break;
    if (!node)
        ERROR(("object was not registered"));
...
void checkpt_raw(const void* data, size_t n_byte) {
    ...
    if (n_byte)
        checkpt_write(checkpt, data, n_byte);
}
...
void restore_raw(void* data, size_t n_byte) {
    ...
    if (n_byte)
        checkpt_read(restore, data, n_byte);
}

/* Composiite checkpt helpers */

void checkpt_data(const void* _data,
                  size_t sz_ele,
                  size_t str_ele,
                  size_t n_ele,
                  size_t max_ele,
    ...
    if (!data && n_ele * sz_ele > 0)
        ERROR(("NULL data"));
    if (n_ele > max_ele || sz_ele > str_ele)
        ERROR(("bad data layout"));

    /* Write the data header */

    CHECKPT_VAL(size_t, 0xDA7A);
    CHECKPT_VAL(size_t, sz_ele);
    CHECKPT_VAL(size_t, str_ele);
    CHECKPT_VAL(size_t, n_ele);
    CHECKPT_VAL(size_t, max_ele);
    CHECKPT_VAL(size_t, align);

    /* Write out the individual elements */

    for (n = 0; n < n_ele; n++)
        checkpt_raw(data + n * str_ele, sz_ele);
}
...
void* restore_data(void) {
    ...
    if (n != 0xDA7A)
        ERROR(("malformed checkpt (expected a data header)"));
    RESTORE_VAL(size_t, sz_ele);
    RESTORE_VAL(size_t, str_ele);
    RESTORE_VAL(size_t, n_ele);
    RESTORE_VAL(size_t, max_ele);
    ...
    if (align == 0)
        MALLOC(data, max_ele * str_ele);
    else
        MALLOC_ALIGNED(data, max_ele * str_ele, align);

    /* And read in the checkpointed elements */

    for (n = 0; n < n_ele; n++)
        restore_raw(data + n * str_ele, sz_ele);
    return data;
}
...
char* restore_str(void) {
    ...
    if (len == 0x2c11577)
        return NULL; /* NULL string received */
    if (len != 0x577)
...
void* restore_fptr(void) {
    ...
    RESTORE_VAL(size_t, id);
    if (!id)
        ERROR(("malformed checkpt (expected an object identifier)"));

    return (void*)id;
}

void* reanimate_fptr(void* ptr) {
    if (!ptr)
        return NULL;
    ptr = object_ptr((size_t)ptr);
    if (!ptr)
        ERROR(
            ("Unable to reanimate a pointer.  Mostly likely, either the "
...
void checkpt_sym(const void* saddr) {
    ...
    if (!saddr)
        ...
        if (first_time) {
            if (!world_rank)
                WARNING((
                    "Checkpointing was compiled without reverse symbol table "
                    "lookup support and has been asked to checkpoint at least "
                    "one symbol.  Checkpointing will likely only work "
                    "correctly "
...
void* restore_sym(void) {
    ...
    if (type == 0x2C11513B)
        return NULL; /* NULL symbol header */
    if (type != 0xADD7513B)
...
static void* find_saddr(const char* sname, const char* fname) {
    void *saddr, *fbase;
    const char* err;
    ...
    if (0 && fname) {
        ...
        err   = dlerror();
        if (err)
            WARNING(("dlopen(%s,RTLD_LAZY): %s", fname, err));
        else {
            dlerror();
...
void checkpt_sym(const void* saddr) {
    ...
    if (saddr == find_saddr(dli->dli_sname, dli->dli_fname)) {
        ...
        WARNING(
            ("Unable to find a safely writable symbol that corresponds to "
             "address %lx (the closest match was \"%s\" from \"%s\").  "
             "Writing out the raw address instead and keeping my fingers "
             "crossed.\n",
             (unsigned long)saddr, !dli->dli_sname ? "(null)" : dli->dli_sname,
             !dli->dli_fname ? "(null)" : dli->dli_fname));
...
void* restore_sym(void) {
    ...
    switch (type) {
        ...
        case 0x577513B: /* string symbol */
            ...
            if (!saddr)
                ERROR((
                    "Unable to resolve the symbol \"%s\" (from \"%s\") in the "
                    "current symbol table.  When this symbol was written, it "
                    "was "
                    "resolvable in old symbol table.  Thus, make sure you have "
                    "not changed the application to eliminate this symbol or "
                    "give "
                    "it a slightly different name, that your paths to various "
                    "libraries are set correctly, that the same library "
                    "versions "
        ...
        case 0xADD7513B: /* address symbol */
            ...
            if (saddr == find_saddr(dli->dli_sname, dli->dli_fname))
                WARNING(
                    ("Read a symbol that was saved unsafely as the symbol's "
                     ...
                     "corresponds to the saved symbol.",
                     (unsigned long)saddr, dli->dli_sname, dli->dli_fname));
            else
                WARNING(
                    ("Read a symbol that was saved unsafely as the symbol's "
                     "address (%lx) in the old symbol table.  This address was "
                     "not "
