...
#define EXEC_PIPELINES(name, args, str)                                    \
    ...
    {                                                                      \
            name##_pipeline_v16(args + id * sizeof(*args) * str, id,       \
                                N_PIPELINE);                               \
        }                                                                  \
    }                                                                      \
...
#define EXEC_PIPELINES(name, args, str)                                    \
    ...
    for( int id = 0; id < N_PIPELINE; id++ )                               \
    {                                                                      \
            name##_pipeline_v8(args + id * sizeof(*args) * str, id,        \
                               N_PIPELINE);                                \
        }                                                                  \
    }                                                                      \
    name##_pipeline_scalar(args + str * N_PIPELINE, N_PIPELINE, N_PIPELINE);
...
#define EXEC_PIPELINES(name, args, str)                                    \
    ...
    for( int id = 0; id < N_PIPELINE; id++ )                               \
    {                                                                      \
            name##_pipeline_v4(args + id * sizeof(*args) * str, id,        \
                               N_PIPELINE);                                \
        }                                                                  \
    }                                                                      \
    name##_pipeline_scalar(args + str * N_PIPELINE, N_PIPELINE, N_PIPELINE);
...
#define EXEC_PIPELINES(name, args, str)                                    \
    ...
    for( int id = 0; id < N_PIPELINE; id++ )                               \
    {                                                                      \
            name##_pipeline_scalar(args + id * sizeof(*args) * str, id,    \
                                   N_PIPELINE);                            \
        }                                                                  \
    }                                                                      \
    name##_pipeline_scalar(args + str * N_PIPELINE, N_PIPELINE, N_PIPELINE);
