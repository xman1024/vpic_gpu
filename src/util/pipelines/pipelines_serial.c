...
pipeline_dispatcher_t* restore_serial(void) {
    ...
    if (serial.n_pipeline != n_pipeline)
        ERROR(
            ("--serial.n_pipeline changed between checkpt (%i) and "
             "restore (%i)",
...
static void serial_boot(int* pargc, char*** pargv) {
    if (serial.n_pipeline != 0)
        ERROR(("Serial dispatcher already booted!"));
    serial.n_pipeline =
        strip_cmdline_int(pargc, pargv, "--serial.n_pipeline", 1);
    if (serial.n_pipeline < 1 || serial.n_pipeline > MAX_PIPELINE)
        ERROR(
            ("Invalid number of pipelines requested (%i)", serial.n_pipeline));
    REGISTER_OBJECT(&serial, checkpt_serial, restore_serial, NULL);
    Busy = 0;
}

static void serial_dispatch(pipeline_func_t func, void* args, int sz, int str) {
    int id;

    if (serial.n_pipeline == 0)
        ERROR(("Boot serial dispatcher first!"));
    if (Busy)
        ERROR(("Pipelines are busy!"));
    Busy = 1;
    for (id = 0; id < serial.n_pipeline; id++)
        if (func)
            func(((char*)args) + id * sz * str, id, serial.n_pipeline);
}
