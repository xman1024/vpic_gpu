...
static void omp_boot(int* pargc, char*** pargv) {
    int n_pipeline;

    if (omp_helper.n_pipeline != 0)
        ERROR(("OMP container has already booted"));
    ...
    int dispatch_to_host =
        strip_cmdline_int(pargc, pargv, "--dispatch_to_host", 1);
...
omp_util( int * pargc,
          char *** pargv )
{
  const char * f_dump = strip_cmdline_string( pargc, pargv, "--dump-fields",
"none.txt" ); const char * e_dump = strip_cmdline_string( pargc, pargv,
"--dump-energies", "none.txt" );
}
*/

omp_container_t omp_helper = {0, 0, omp_boot};
