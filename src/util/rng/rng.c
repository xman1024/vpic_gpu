...
static void adjust_rng(rng_t* RESTRICT r) {
    static const uint32_t parity[4] = {SFMT_PARITY0, SFMT_PARITY1, SFMT_PARITY2,
                                       SFMT_PARITY3};
    uint32_t bit                    = 0;
    int n;
...
rng_t* seed_rng(rng_t* RESTRICT r, int seed) {
    int n;
    if (!r)
        ERROR(("Bad args"));
    u(0) = (uint32_t)seed;
...
#define _(type, prefix, state_prefix, is_signed)                        \
    type prefix##rand(rng_t* RESTRICT r) {                              \
        type x;                                                         \
        RNG_NEXT(x, type, r, state_prefix, is_signed);                  \
        return x;                                                       \
    }                                                                   \
                                                                        \
    type* prefix##rand_fill(rng_t* RESTRICT r, type* RESTRICT x,        \
                            size_t str_ele, size_t n_ele) {             \
        size_t n;                                                       \
        if (!n_ele)                                                     \
            return x;                                                   \
        if (!r || !x)                                                   \
...
_(short, h, uh, 1) _(unsigned short, uh, uh, 0) _(int, i, ui, 1) _(
    unsigned int,
    ui,
    ui,
    0) _(long, l, ul, 1) _(unsigned long, ul, ul, 0) _(int8_t, i8, u8, 1)
    _(uint8_t, u8, u8, 0) _(int16_t, i16, u16, 1) _(uint16_t, u16, u16, 0)
        _(int32_t, i32, u32, 1) _(uint32_t, u32, u32, 0) _(int64_t, i64, u64, 1)
...
#define _(type, prefix, variant, state_type, state_prefix)                  \
    type prefix##rand##variant(rng_t* RESTRICT r) {                         \
        state_type u;                                                       \
        RNG_NEXT(u, state_type, r, state_prefix, 0);                        \
        return conv_##prefix##rand##variant(u);                             \
    }                                                                       \
                                                                            \
    type* prefix##rand##variant##_fill(rng_t* RESTRICT r, type* RESTRICT x, \
                                       size_t str_ele, size_t n_ele) {      \
        state_type u;                                                       \
        size_t n;                                                           \
        if (!n_ele)                                                         \
...
#error "frandn_table.h does not match frandn()"
        ...
        if (LIKELY(x < frandn_zig_x[i]))
            break;  // Vast majority of the time
...
float* frandn_fill(rng_t* RESTRICT r,
                   float* RESTRICT x,
                   size_t str_ele,
                   size_t n_ele) {
    size_t n;
    ...
    if (!r || !x)
        ERROR(("Bad args"));
    for (n = 0; n < n_ele; n++)
        x[n * str_ele] = frandn(r);
    return x;
}

#include "drandn_table.h"

double drandn(rng_t* RESTRICT r) {
    uint64_t a, i, j, s;
    double x, y;
...
double* drandn_fill(rng_t* RESTRICT r,
                    double* RESTRICT x,
                    size_t str_ele,
                    size_t n_ele) {
    size_t n;
...
float frande(rng_t* RESTRICT r) {
    uint32_t a;
    RNG_NEXT(a, uint32_t, r, u32, 0);
    return -logf(conv_frand_c1(a));
}

float* frande_fill(rng_t* RESTRICT r,
                   float* RESTRICT x,
                   size_t str_ele,
                   size_t n_ele) {
    size_t n;
    ...
    if (!r || !x)
        ERROR(("Bad args"));
    for (n = 0; n < n_ele; n++)
        x[n * str_ele] = frande(r);
    return x;
}

double drande(rng_t* RESTRICT r) {
    uint64_t a;
    RNG_NEXT(a, uint64_t, r, u64, 0);
    return -log(conv_drand_c1(a));
}

double* drande_fill(rng_t* RESTRICT r,
                    double* RESTRICT x,
                    size_t str_ele,
                    size_t n_ele) {
    size_t n;
    ...
    if (!r || !x)
        ERROR(("Bad args"));
    for (n = 0; n < n_ele; n++)
        x[n * str_ele] = drande(r);
    return x;
}

/* Miscellaneous */

int* randperm(rng_t* RESTRICT r, int* RESTRICT x, int n) {
    uint32_t a, d;
    int t, i, j;
...
#define SWAP_IJ                                                      \
    for (k = 0; k < sz_ele; k++)                                     \
    t = x[i * str_ele + k], x[i * str_ele + k] = x[j * str_ele + k], \
    x[j * str_ele + k] = t
