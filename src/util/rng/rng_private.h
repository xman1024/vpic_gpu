...
#define DECL_SFMT \
    __m128i a_u,  \
        mask = _mm_setr_epi32(SFMT_MASK0, SFMT_MASK1, SFMT_MASK2, SFMT_MASK3)
...
#define DECL_SFMT uint32_t x0, x1, x2, x3, y0, y1, y2, y3
...
#define SFMT(a, b, c, d)                                                    \
    ...
    y3 = (c.u3 >> SFMT_R2A);                                                \
    a.u0 ^=                                                                 \
        (x0 ^ ((b.u0 >> SFMT_R1) & SFMT_MASK0)) ^ (y0 ^ (d.u0 << SFMT_L1)); \
    a.u1 ^=                                                                 \
        (x1 ^ ((b.u1 >> SFMT_R1) & SFMT_MASK1)) ^ (y1 ^ (d.u1 << SFMT_L1)); \
    a.u2 ^=                                                                 \
        (x2 ^ ((b.u2 >> SFMT_R1) & SFMT_MASK2)) ^ (y2 ^ (d.u2 << SFMT_L1)); \
...
#define conv_drand_c(u64) \
    ((((u64) >> 11) + ((u64)&1)) * (1. / 9007199254740992.))
