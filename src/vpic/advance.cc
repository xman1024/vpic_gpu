...
int vpic_simulation::advance(void) {
    ...
    if (collision_op_list)
        TIC apply_collision_op_list(collision_op_list);
    TOC(collision_model, 1);
    TIC user_particle_collisions();
    TOC(user_particle_collisions, 1);
    ...
    TIC user_particle_injection();
    TOC(user_particle_injection, 1);
    ...
    LIST_FOR_EACH(sp, species_list) {
        if (sp->nm && verbose)
            WARNING(
                ("Removing %i particles associated with unprocessed %s movers "
                 "(increase num_comm_round)",
                 sp->nm, sp->name));
        // Drop the particles that have unprocessed movers due to a user defined
        // boundary condition. Particles of this type with unprocessed movers
        // are in the list of particles and move_p has set the voxel in the
        // particle to 8*voxel + face. This is an incorrect voxel index and in
    ...
    TOC(clear_jf, 1);
    if (species_list)
        TIC unload_accumulator_array(field_array, accumulator_array);
    TOC(unload_accumulator, 1);
    TIC FAK->synchronize_jf(field_array);
    ...
    TIC FAK->advance_e(field_array, 1.0);
    TOC(advance_e, 1);
    ...
    TIC user_field_injection();
    TOC(user_field_injection, 1);
    ...
    if ((clean_div_e_interval > 0) && ((step() % clean_div_e_interval) == 0)) {
        ...
        TIC FAK->synchronize_rho(field_array);
        TOC(synchronize_rho, 1);

        for (int round = 0; round < num_div_e_round; round++) {
            TIC FAK->compute_div_e_err(field_array);
            TOC(compute_div_e_err, 1);
            if (round == 0 || round == num_div_e_round - 1) {
                TIC err = FAK->compute_rms_div_e_err(field_array);
                TOC(compute_rms_div_e_err, 1);
                if (rank() == 0)
                    MESSAGE(("%s rms error = %e (charge/volume)",
                             round == 0 ? "Initial" : "Cleaned", err));
            }
            TIC FAK->clean_div_e(field_array);
            TOC(clean_div_e, 1);
        }
    }
    ...
    if ((clean_div_b_interval > 0) && ((step() % clean_div_b_interval) == 0)) {
        ...
        for (int round = 0; round < num_div_b_round; round++) {
            TIC FAK->compute_div_b_err(field_array);
            TOC(compute_div_b_err, 1);
            if (round == 0 || round == num_div_b_round - 1) {
                TIC err = FAK->compute_rms_div_b_err(field_array);
                TOC(compute_rms_div_b_err, 1);
                if (rank() == 0)
                    MESSAGE(("%s rms error = %e (charge/volume)",
                             round == 0 ? "Initial" : "Cleaned", err));
            }
            TIC FAK->clean_div_b(field_array);
            TOC(clean_div_b, 1);
        }
    }
    ...
    if ((sync_shared_interval > 0) && ((step() % sync_shared_interval) == 0)) {
        if (rank() == 0)
            MESSAGE(("Synchronizing shared tang e, norm b, rho_b"));
        TIC err = FAK->synchronize_tang_e_norm_b(field_array);
    ...
    if (species_list)
        TIC load_interpolator_array(interpolator_array, field_array);
    ...
    if ((status_interval > 0) && ((step() % status_interval) == 0)) {
        if (rank() == 0)
            MESSAGE(("Completed step %i of %i", step(), num_step));
