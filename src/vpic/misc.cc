...
void vpic_simulation::inject_particle(species_t* sp,
                                      double x,
                                      double y,
                                      double z,
                                      double ux,
    ...
    const double x0 = (double)grid->x0, y0 = (double)grid->y0,
                 z0 = (double)grid->z0;
    const double x1 = (double)grid->x1, y1 = (double)grid->y1,
                 z1 = (double)grid->z1;
    ...
    if ((x < x0) | (x > x1) | ((x == x1) & (grid->bc[BOUNDARY(1, 0, 0)] >= 0)))
        return;
    if ((y < y0) | (y > y1) | ((y == y1) & (grid->bc[BOUNDARY(0, 1, 0)] >= 0)))
        return;
    if ((z < z0) | (z > z1) | ((z == z1) & (grid->bc[BOUNDARY(0, 0, 1)] >= 0)))
        return;
...
#define SETIVAR(V, A, S)                                      \
    do {                                                      \
        V = (A);                                              \
        if (rank() == 0)                                      \
            log_printf("*** Modifying %s to value %d", S, A); \
    } while (0)
...
#define SETDVAR(V, A, S)                                       \
    do {                                                       \
        V = (A);                                               \
        if (rank() == 0)                                       \
            log_printf("*** Modifying %s to value %le", S, A); \
    } while (0)

#define ITEST(V, N, A)                     \
    if (sscanf(line, N " %d", &iarg) == 1) \
    SETIVAR(V, A, N)

#define DTEST(V, N, A)                      \
    if (sscanf(line, N " %le", &darg) == 1) \
    SETDVAR(V, A, N)

void vpic_simulation::modify(const char* fname) {
    FILE* handle = NULL;
    char line[128];
    int iarg    = 0;
    double darg = 0;
