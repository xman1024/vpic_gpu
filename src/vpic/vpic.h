...
const uint32_t stress_tensor(1 << 8 | 1 << 9 | 1 << 10 | 1 << 11 | 1 << 12 |
                             1 << 13);
...
----------------------------------------------------------------------------*/
enum DumpFormat { band = 0, band_interleave = 1 };  // enum DumpFormat
...
class vpic_simulation {
   ...
   protected:
    ...
    void hydro_header(const char* speciesname,
                      const char* hbase,
    ...
    inline int rank() { return world_rank; }

    inline int nproc() { return world_size; }

    inline void barrier() { mp_barrier(); }
    ...
    inline interpolator_t& interpolator(const int ix,
                                        const int iy,
                                        const int iz) {
    ...
    inline void define_units(float cvac, float eps0) {
        grid->cvac = cvac;
        grid->eps0 = eps0;
    }

    inline void define_timestep(float dt, double t0 = 0, int64_t step = 0) {
        grid->t0   = t0;
        grid->dt   = (float)dt;
        grid->step = step;
    }
    ...
    inline void define_periodic_grid(double xl,
                                     double yl,
                                     double zl,
                                     double xh,
                                     double yh,
        ...
        py = size_t(gpy);
        pz = size_t(gpz);
        partition_periodic_box(grid, xl, yl, zl, xh, yh, zh, (int)gnx, (int)gny,
                               (int)gnz, (int)gpx, (int)gpy, (int)gpz);
    }

    inline void define_absorbing_grid(double xl,
                                      double yl,
                                      double zl,
                                      double xh,
                                      double yh,
        ...
        py = size_t(gpy);
        pz = size_t(gpz);
        partition_absorbing_box(grid, xl, yl, zl, xh, yh, zh, (int)gnx,
                                (int)gny, (int)gnz, (int)gpx, (int)gpy,
                                (int)gpz, pbc);
    }

    inline void define_reflecting_grid(double xl,
                                       double yl,
                                       double zl,
                                       double xh,
                                       double yh,
        ...
        py = size_t(gpy);
        pz = size_t(gpz);
        partition_metal_box(grid, xl, yl, zl, xh, yh, zh, (int)gnx, (int)gny,
                            (int)gnz, (int)gpx, (int)gpy, (int)gpz);
    }
    ...
    inline material_t* define_material(const char* name,
                                       double eps,
                                       double mu    = 1,
                                       double sigma = 0,
                                       double zeta  = 0) {
        return append_material(material(name, eps, eps, eps, mu, mu, mu, sigma,
    ...
    inline material_t* define_material(const char* name,
        ...
        return append_material(
            material(name, epsx, epsy, epsz, mux, muy, muz, sigmax, sigmay,
    ...
    inline void define_field_array(field_array_t* fa = NULL, double damp = 0) {
        ...
        field_array =
            fa ? fa : new_standard_field_array(grid, material_list, damp);
        ...
        mp_size_recv_buffer(grid->mp, BOUNDARY(-1, 0, 0),
                            ny1 * nz1 * sizeof(hydro_t));
        mp_size_recv_buffer(grid->mp, BOUNDARY(1, 0, 0),
                            ny1 * nz1 * sizeof(hydro_t));
        mp_size_recv_buffer(grid->mp, BOUNDARY(0, -1, 0),
                            nz1 * nx1 * sizeof(hydro_t));
        mp_size_recv_buffer(grid->mp, BOUNDARY(0, 1, 0),
                            nz1 * nx1 * sizeof(hydro_t));
        mp_size_recv_buffer(grid->mp, BOUNDARY(0, 0, -1),
                            nx1 * ny1 * sizeof(hydro_t));
        mp_size_recv_buffer(grid->mp, BOUNDARY(0, 0, 1),
                            nx1 * ny1 * sizeof(hydro_t));

        mp_size_send_buffer(grid->mp, BOUNDARY(-1, 0, 0),
                            ny1 * nz1 * sizeof(hydro_t));
        mp_size_send_buffer(grid->mp, BOUNDARY(1, 0, 0),
                            ny1 * nz1 * sizeof(hydro_t));
        mp_size_send_buffer(grid->mp, BOUNDARY(0, -1, 0),
                            nz1 * nx1 * sizeof(hydro_t));
        mp_size_send_buffer(grid->mp, BOUNDARY(0, 1, 0),
                            nz1 * nx1 * sizeof(hydro_t));
        mp_size_send_buffer(grid->mp, BOUNDARY(0, 0, -1),
                            nx1 * ny1 * sizeof(hydro_t));
        mp_size_send_buffer(grid->mp, BOUNDARY(0, 0, 1),
                            nx1 * ny1 * sizeof(hydro_t));
    }
    ...
    inline species_t* define_species(const char* name,
                                     double q,
                                     double m,
                                     double max_local_np,
                                     double max_local_nm,
        ...
        // Based on the twice the number of particles expected to hit the
        // boundary of a wpdt=0.2 / dx=lambda species in a 3x3x3 domain
        if (max_local_nm < 0) {
            max_local_nm = 2 * max_local_np / 25;
            if (max_local_nm < 16 * (MAX_PIPELINE + 1))
                max_local_nm = 16 * (MAX_PIPELINE + 1);
        }
        return append_species(
            species(name, (float)q, (float)m, (size_t)max_local_np,
                    (size_t)max_local_nm, (int)sort_interval,
                    (int)sort_out_of_place, grid),
            &species_list);
    }
    ...
    void inject_particle(species_t* sp,
                         double x,
                         double y,
                         double z,
                         double ux,
    ...
    inline void inject_particle_raw(species_t* RESTRICT sp,
                                    float dx,
                                    float dy,
                                    float dz,
                                    int32_t i,
    ...
    inline double courant_length(double lx,
                                 double ly,
                                 double lz,
                                 double nx,
                                 double ny,
