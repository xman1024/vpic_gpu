...
#define WRITE_HEADER_V0(dump_type, sp_id, q_m, fileIO) \
    do {                                               \
...
#define WRITE_ARRAY_HEADER(p, ndim, dim, fileIO) \
    do {                                         \
...
#define WRITE_STRING(string, fileIO)                  \
    do {                                              \
        ...
        if (string)                                   \
            __WRITE_STRING_len = strlen(string);      \
...
#define F_WRITE_HEADER_V0(dump_type, sp_id, q_m, fileIO) \
    do {                                                 \
        /* Binary compatibility information */           \
...
#define F_WRITE_HEADER_PAR(dump_type, sp_id, q_m, fileIO) \
    do {                                                  \
        /* Binary compatibility information */            \
        F_WRITE(char, CHAR_BIT, fileIO);                  \
        F_WRITE(char, sizeof(short int), fileIO);         \
...
#define F_WRITE_ARRAY_HEADER(psiz, ndim, dim, fileIO) \
    do {                                              \
...
#define F_WRITE(type, value, fileIO)        \
    do {                                    \
        type __F_WRITE_tmp = (type)(value); \
        fileIO.write(&__F_WRITE_tmp, 1);    \
    } while (0)

#define F_READ(type, value, fileIO)    \
    do {                               \
        type __F_READ_tmp;             \
