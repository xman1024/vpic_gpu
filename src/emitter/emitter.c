...
void checkpt_emitter_internal(const emitter_t* RESTRICT e) {
    CHECKPT(e, 1);
    CHECKPT_SYM(e->emit);
...
emitter_t* restore_emitter_internal(void* params) {
    emitter_t* e;
    RESTORE(e);
    e->params = params;
    RESTORE_SYM(e->emit);
...
emitter_t* new_emitter_internal(void* params,
                                emit_func_t emit,
...
int num_emitter(const emitter_t* RESTRICT e_list) {
...
void apply_emitter_list(emitter_t* RESTRICT e_list) {
    emitter_t* e;
...
emitter_t* append_emitter(emitter_t* e, emitter_t** e_list) {
    ...
    if (e->next)
        ERROR(("Emitter already in a list"));
    e->next = *e_list;
    *e_list = e;
    return e;
}

int32_t* ALIGNED(128) size_emitter(emitter_t* RESTRICT e, int n_component) {
    if (!e || e->n_component || n_component < 0)
        ERROR(("Bad args"));
