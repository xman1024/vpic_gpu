...
void emit_child_langmuir(child_langmuir_t* RESTRICT cl,
    ...
    /**/ particle_t* RESTRICT ALIGNED(128) p        = sp->device_p0;
...
#define EMIT_PARTICLES(X, Y, Z, dir)                                    \
    ...
    if (dir qsp * w > thresh) { /* This face can emit */                \
        ...
        switch (EXTRACT_COMPONENT_TYPE(cc)) {
            case BOUNDARY(-1, 0, 0):
                EMIT_PARTICLES(x, y, z, +) break;
            case BOUNDARY(0, -1, 0):
                EMIT_PARTICLES(y, z, x, +) break;
            case BOUNDARY(0, 0, -1):
                EMIT_PARTICLES(z, x, y, +) break;
            case BOUNDARY(1, 0, 0):
                EMIT_PARTICLES(x, y, z, -) break;
            case BOUNDARY(0, 1, 0):
                EMIT_PARTICLES(y, z, x, -) break;
            case BOUNDARY(0, 0, 1):
                EMIT_PARTICLES(z, x, y, -) break;
            default: /* Not a cell face ... do not emit */
                break;
        }
...
#undef EMIT_PARTICLES
    ...
    if (np_skipped)
        WARNING(
            ("Insufficient local particle storage.  Did not emit %i "
             "particles in emit_child_langmuir",
             np_skipped));
    if (nm_skipped)
        WARNING(
            ("Insufficient local particle mover storage.  Did not age %i "
             "emitted particles in emit_child_langmuir",
             nm_skipped));
}

void checkpt_child_langmuir(const emitter_t* e) {
...
emitter_t* restore_child_langmuir(void) {
    child_langmuir_t* cl;
    RESTORE(cl);
    RESTORE_PTR(cl->sp);
    RESTORE_PTR(cl->ia);
...
void delete_child_langmuir(emitter_t* e) {
    FREE(e->params);
...
emitter_t* child_langmuir(/**/ species_t* RESTRICT sp,
    ...
    if (!sp || !ia || !fa || !aa || !rp || sp->g != ia->g || sp->g != fa->g ||
        sp->g != aa->g || n_emit_per_face < 1 || ut_para < 0 || ut_perp < 0 ||
        thresh_e_norm < 0)
        ERROR(("Bad args"));
    ...
    cl->norm            = norm;
    return new_emitter_internal(cl, (emit_func_t)emit_child_langmuir,
                                delete_child_langmuir,
                                (checkpt_func_t)checkpt_child_langmuir,
                                (restore_func_t)restore_child_langmuir, NULL);
}
