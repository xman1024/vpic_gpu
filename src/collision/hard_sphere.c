...
float hard_sphere_fluid_rate_constant(const hard_sphere_t* RESTRICT hs,
    ...
    return sqrtf(
        (hs->alpha_Kt2ut4 + ur2 * (hs->beta_Kt2ut2 + ur2 * hs->gamma_Kt2)) /
        (hs->ut2 + ur2 * gamma));
}

/* The particle-particle case is much easier theoretically. */

float hard_sphere_rate_constant(const hard_sphere_t* RESTRICT hs,
                                const species_t* RESTRICT spi,
...
void hard_sphere_fluid_collision(const hard_sphere_t* RESTRICT hs,
                                 const species_t* RESTRICT spi,
...
void hard_sphere_collision(const hard_sphere_t* RESTRICT hs,
                           const species_t* RESTRICT spi,
                           const species_t* RESTRICT spj,
                           /**/ particle_t* RESTRICT pi,
...
void checkpt_hard_sphere(const hard_sphere_t* hs) {
    CHECKPT(hs, 1);
}

hard_sphere_t* restore_hard_sphere(void) {
    hard_sphere_t* hs;
    RESTORE(hs);
    return hs;
}
...
collision_op_t* hard_sphere_fluid(
    ...
    if (n0 < 0 || kT0 < 0 || m0 <= 0 || r0 < 0 || !sp || sp->m <= 0 || rsp < 0)
        ERROR(("Bad args"));
    ...
    hs->alpha_Kt2ut4 =
        (8. / M_PI) * (hs->Kc * n0 * hs->Kc * n0) * (hs->ut2 * hs->ut2);
    hs->beta_Kt2ut2 =
        (4. / (12. - 3. * M_PI)) * (hs->Kc * n0 * hs->Kc * n0) * hs->ut2;
    hs->gamma_Kt2 =
        ((3. * M_PI - 8.) / (24. - 6 * M_PI)) * (hs->Kc * n0 * hs->Kc * n0);
    hs->ut2 += FLT_MIN;

    REGISTER_OBJECT(hs, checkpt_hard_sphere, restore_hard_sphere, NULL);
    return unary_collision_model(
        name, (unary_rate_constant_func_t)hard_sphere_fluid_rate_constant,
        (unary_collision_func_t)hard_sphere_fluid_collision, hs, sp, rp,
        interval);
}
...
collision_op_t* hard_sphere(const char* RESTRICT name, /* Model name */
    ...
    if (!spi || spi->m <= 0 || ri < 0 || !spj || spj->m <= 0 || rj < 0 ||
        spi->g != spj->g)
        ERROR(("Bad args"));
    ...
    return binary_collision_model(
        name, (binary_rate_constant_func_t)hard_sphere_rate_constant,
        (binary_collision_func_t)hard_sphere_collision, hs, spi, spj, rp,
        sample, interval);
}
