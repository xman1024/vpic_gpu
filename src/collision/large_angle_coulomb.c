...
float large_angle_coulomb_fluid_rate_constant(
    ...
    return sqrtf(
        (lac->alpha_Kt2ut4 + ur2 * (lac->beta_Kt2ut2 + ur2 * lac->gamma_Kt2)) /
        (lac->ut2 + ur2 * gamma));
}

float large_angle_coulomb_rate_constant(const large_angle_coulomb_t* RESTRICT
                                            lac,
                                        const species_t* RESTRICT spi,
...
void large_angle_coulomb_fluid_collision(const large_angle_coulomb_t* RESTRICT
                                             lac,
                                         const species_t* RESTRICT spi,
...
void large_angle_coulomb_collision(const large_angle_coulomb_t* RESTRICT lac,
                                   const species_t* RESTRICT spi,
                                   const species_t* RESTRICT spj,
                                   /**/ particle_t* RESTRICT pi,
...
void checkpt_large_angle_coulomb(const large_angle_coulomb_t* lac) {
    CHECKPT(lac, 1);
}

large_angle_coulomb_t* restore_large_angle_coulomb(void) {
...
collision_op_t* large_angle_coulomb_fluid(
    ...
    if (n0 < 0 || kT0 < 0 || !q0 || m0 <= 0 || !sp || !sp->q || sp->m <= 0 ||
        bmax < 0)
        ERROR(("Bad args"));

    MALLOC(lac, 1);

    lac->cc = (4. * M_PI * sp->g->eps0 * sp->m * m0 * sp->g->cvac *
               sp->g->cvac * bmax) /
              ((sp->m + m0) * sp->q * q0);
    ...
    lac->alpha_Kt2ut4 =
        (8. / M_PI) * (lac->Kc * n0 * lac->Kc * n0) * (lac->ut2 * lac->ut2);
    lac->beta_Kt2ut2 =
        (4. / (12. - 3. * M_PI)) * (lac->Kc * n0 * lac->Kc * n0) * lac->ut2;
    lac->gamma_Kt2 =
        ((3. * M_PI - 8.) / (24. - 6 * M_PI)) * (lac->Kc * n0 * lac->Kc * n0);
    lac->ut2 += FLT_MIN;

    REGISTER_OBJECT(lac, checkpt_large_angle_coulomb,
                    restore_large_angle_coulomb, NULL);
    return unary_collision_model(
        name,
        (unary_rate_constant_func_t)large_angle_coulomb_fluid_rate_constant,
        (unary_collision_func_t)large_angle_coulomb_fluid_collision, lac, sp,
        rp, interval);
}

collision_op_t* large_angle_coulomb(
    const char* RESTRICT name, /* Model name */
    ...
    if (!spi || !spi->q || spi->m <= 0 || !spj || !spj->q || spj->m <= 0 ||
        spi->g != spj->g)
        ERROR(("Bad args"));

    MALLOC(lac, 1);
    CLEAR(lac, 1);
    lac->cc = (4. * M_PI * spi->g->eps0 * spi->m * spj->m * spi->g->cvac *
               spi->g->cvac * bmax) /
    ...
    REGISTER_OBJECT(lac, checkpt_large_angle_coulomb,
                    restore_large_angle_coulomb, NULL);
    return binary_collision_model(
        name, (binary_rate_constant_func_t)large_angle_coulomb_rate_constant,
        (binary_collision_func_t)large_angle_coulomb_collision, lac, spi, spj,
        rp, sample, interval);
}
