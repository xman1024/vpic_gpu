...
void checkpt_collision_op_internal(const collision_op_t* RESTRICT cop) {
    CHECKPT(cop, 1);
...
collision_op_t* new_collision_op_internal(void* params,
...
void apply_collision_op_list(collision_op_t* cop_list) {
    collision_op_t* cop;
    LIST_FOR_EACH(cop, cop_list) cop->apply(cop->params);
}

void delete_collision_op_list(collision_op_t* cop_list) {
    collision_op_t* cop;
    while (cop_list) {
        cop      = cop_list;
