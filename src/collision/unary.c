...
void apply_unary_collision_model(unary_collision_model_t* cm) {
    if (cm->interval < 1 || (cm->sp->g->step % cm->interval)) {
        return;
    }
...
void checkpt_unary_collision_model(const collision_op_t* cop) {
...
collision_op_t* restore_unary_collision_model(void) {
...
void delete_unary_collision_model(collision_op_t* cop) {
...
collision_op_t* unary_collision_model(const char* RESTRICT name,
    ...
    return new_collision_op_internal(
        cm, (collision_op_func_t)apply_unary_collision_model,
