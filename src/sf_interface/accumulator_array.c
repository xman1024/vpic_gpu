...
void checkpt_accumulator_array(const accumulator_array_t* aa) {
    CHECKPT(aa, 1);
    CHECKPT_ALIGNED(aa->a, (size_t)(aa->n_pipeline + 1) * (size_t)aa->stride,
                    128);
    CHECKPT_PTR(aa->g);
}

accumulator_array_t* restore_accumulator_array(void) {
    ...
    if (aa->n_pipeline != aa_n_pipeline())
        ERROR(
            ("Number of accumulators restored is not the same as the number of "
...
accumulator_array_t* new_accumulator_array(grid_t* g) {
    ...
    aa->g          = g;
    MALLOC_ALIGNED(aa->a, (size_t)(aa->n_pipeline + 1) * (size_t)aa->stride,
                   128);
...
void delete_accumulator_array(accumulator_array_t* aa) {
    if (!aa)
        return;
