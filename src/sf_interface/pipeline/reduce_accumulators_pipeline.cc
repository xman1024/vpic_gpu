...
void reduce_accumulators_pipeline_scalar(accumulators_pipeline_args_t* args,
    ...
    DISTRIBUTE(args->n, accumulators_n_block, pipeline_rank, n_pipeline, i, i1);
...
#define O1(k) A(k) B(k, 1) C(k, v0 + v1)
#define O2(k) A(k) B(k, 1) B(k, 2) C(k, (v0 + v1) + v2)
#define O3(k) A(k) B(k, 1) B(k, 2) B(k, 3) C(k, (v0 + v1) + (v2 + v3))
#define O4(k) \
    A(k) B(k, 1) B(k, 2) B(k, 3) B(k, 4) C(k, ((v0 + v1) + (v2 + v3)) + v4)
#define O5(k) \
    A(k)      \
    B(k, 1)   \
...
#define O4(k) \
    a[k] = ((a[k] + b[k]) + (b[k + sr] + b[k + 2 * sr])) + /**/ b[k + 3 * sr]
#define O5(k)                                              \
    a[k] = ((a[k] + b[k]) + (b[k + sr] + b[k + 2 * sr])) + \
           /**/ (b[k + 3 * sr] + b[k + 4 * sr])
#define O6(k)                                              \
    a[k] = ((a[k] + b[k]) + (b[k + sr] + b[k + 2 * sr])) + \
...
void reduce_accumulator_array_pipeline(accumulator_array_t* RESTRICT aa) {
    ...
    i0 =
        (VOX(1, 1, 1) / 2) * 2;  // Round i0 down to even for 128B align on Cell
