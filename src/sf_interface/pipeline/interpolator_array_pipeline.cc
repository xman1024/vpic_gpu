...
// particles or fields that can be easily extended to longer SIMD vector
// lengths. Thus, it appears that this file is not a candidate for
// V8_ACCELERATION. #define HAS_V8_PIPELINE
...
void load_interpolator_pipeline_scalar(load_interpolator_pipeline_args_t* args,
    ...
    if (pipeline_rank == n_pipeline)
        return;  // No straggler cleanup needed

    DISTRIBUTE_VOXELS(1, nx, 1, ny, 1, nz, 1, pipeline_rank, n_pipeline, x, y,
                      z, n_voxel);
...
#define LOAD_STENCIL()          \
    ...
    for (; n_voxel; n_voxel--) {
        ...
        w0 = toggle_bits(sgn_1_3,
                         merge(sel_0_1, v4float(pf0->cbx),
                               v4float(pf0->cby)));  // [ w0x -w0x w0y -w0y ]
        w1 = merge(sel_0_1, v4float(pfx->cbx),
