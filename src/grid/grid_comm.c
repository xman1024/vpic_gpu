...
void begin_recv_port(int i, int j, int k, int size, const grid_t* g) {
...
void* ALIGNED(128) end_recv_port(int i, int j, int k, const grid_t* g) {
    int port = BOUNDARY(-i, -j, -k), src = g->bc[port];
    if (src < 0 || src >= world_size)
        return NULL;
...
void* ALIGNED(128)
    size_send_port(int i, int j, int k, int size, const grid_t* g) {
...
void begin_send_port(int i, int j, int k, int size, const grid_t* g) {
    int port = BOUNDARY(i, j, k), dst = g->bc[port];
    if (dst < 0 || dst >= world_size)
        return;
...
void end_send_port(int i, int j, int k, const grid_t* g) {
    int port = BOUNDARY(i, j, k), dst = g->bc[port];
    if (dst < 0 || dst >= world_size)
        return;
