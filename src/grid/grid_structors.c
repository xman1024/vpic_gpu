...
void checkpt_grid(const grid_t* g) {
    CHECKPT(g, 1);
    if (g->range)
        CHECKPT_ALIGNED(g->range, world_size + 1, 16);
    if (g->neighbor)
        CHECKPT_ALIGNED(g->neighbor, 6 * g->nv, 128);
    CHECKPT_PTR(g->mp);
}

grid_t* restore_grid(void) {
    grid_t* g;
    RESTORE(g);
    if (g->range)
        RESTORE_ALIGNED(g->range);
    if (g->neighbor)
        RESTORE_ALIGNED(g->neighbor);
    RESTORE_PTR(g->mp);
    return g;
}
