...
static field_advance_kernels_t sfa_kernels = {
    ...
    clear_jf, synchronize_jf, clear_rhof, synchronize_rho,

    // Initialize interface

    compute_rhob, compute_curl_b,
    ...
    compute_div_e_err, compute_rms_div_e_err, clean_div_e,

    // Magnetic field divergence cleaning interface

    compute_div_b_err, compute_rms_div_b_err, clean_div_b
...
static sfa_params_t* create_sfa_params(grid_t* g,
    ...
    ax = g->nx > 1 ? g->cvac * g->dt * g->rdx : 0;
    ax *= ax;
    ay = g->ny > 1 ? g->cvac * g->dt * g->rdy : 0;
    ay *= ay;
    az = g->nz > 1 ? g->cvac * g->dt * g->rdz : 0;
    az *= az;
    n_mc = 0;
    LIST_FOR_EACH(m, m_list) {
        ...
        if (cg2 >= 1)
            WARNING(
                ("\"%s\" Courant condition estimate = %e", m->name, sqrt(cg2)));
    ...
    LIST_FOR_EACH(m, m_list) {
        ...
        //       mc->decay{x,y,z} = 0 -> Perfect conductor to numerical
        //       precision otherwise            -> Conductive
...
void checkpt_standard_field_array(const field_array_t* fa) {
...
field_array_t* restore_standard_field_array(void) {
    field_array_t* fa;
    sfa_params_t* p;
    RESTORE(fa);
...
void delete_standard_field_array(field_array_t* fa) {
    if (!fa)
        return;
