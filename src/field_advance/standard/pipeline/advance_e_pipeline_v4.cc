...
void advance_e_pipeline_v4(pipeline_args_t* args,
                           int pipeline_rank,
                           int n_pipeline) {
    DECLARE_STENCIL();
    ...
    for (; n_voxel > 3; n_voxel -= 4) {
        ...
        load_4x3_tr(&f00->jfx, &f01->jfx, &f02->jfx, &f03->jfx, f0_jfx, f0_jfy,
                    f0_jfz);
...
#define LOAD_RMU(V, D)                                                  \
    m_f##V##_rmu##D =                                                   \
        ...
        load_4x2_tr(&m[f00->ematx].decayx, &m[f01->ematx].decayx,
                    &m[f02->ematx].decayx, &m[f03->ematx].decayx, m_f0_decayx,
                    m_f0_drivex);

        load_4x2_tr(&m[f00->ematy].decayy, &m[f01->ematy].decayy,
                    &m[f02->ematy].decayy, &m[f03->ematy].decayy, m_f0_decayy,
                    m_f0_drivey);

        load_4x2_tr(&m[f00->ematz].decayz, &m[f01->ematz].decayz,
                    &m[f02->ematz].decayz, &m[f03->ematz].decayz, m_f0_decayz,
                    m_f0_drivez);
...
#undef LOAD_RMU
        ...
        store_4x4_tr(f0_ex, f0_ey, f0_ez, save0, &f00->ex, &f01->ex, &f02->ex,
                     &f03->ex);
