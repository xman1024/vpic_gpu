...
#error \
    "Only include vacuum_energy_f_pipeline.h in vacuum_energy_f_pipeline source files."
#endif
...
#define REDUCE_EN()                                                         \
    en_ex += qepsx * (f0->ex * f0->ex + fy->ex * fy->ex + fz->ex * fz->ex + \
                      fyz->ex * fyz->ex);                                   \
    en_ey += qepsy * (f0->ey * f0->ey + fz->ey * fz->ey + fx->ey * fx->ey + \
                      fzx->ey * fzx->ey);                                   \
    en_ez += qepsz * (f0->ez * f0->ez + fx->ez * fx->ez + fy->ez * fy->ez + \
                      fxy->ez * fxy->ez);                                   \
    en_bx += hrmux * (f0->cbx * f0->cbx + fx->cbx * fx->cbx);               \
    en_by += hrmuy * (f0->cby * f0->cby + fy->cby * fy->cby);               \
    en_bz += hrmuz * (f0->cbz * f0->cbz + fz->cbz * fz->cbz)

void vacuum_energy_f_pipeline_scalar(pipeline_args_t* args,
                                     int pipeline_rank,
                                     int n_pipeline);
