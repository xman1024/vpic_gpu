...
#define DECLARE_STENCIL()                                                 \
    ...
    const float px_muz =                                                  \
        ((nx > 1) ? (1 + damp) * g->cvac * g->dt * g->rdx : 0) * m->rmuz; \
    const float px_muy =                                                  \
        ((nx > 1) ? (1 + damp) * g->cvac * g->dt * g->rdx : 0) * m->rmuy; \
    const float py_mux =                                                  \
        ((ny > 1) ? (1 + damp) * g->cvac * g->dt * g->rdy : 0) * m->rmux; \
    const float py_muz =                                                  \
        ((ny > 1) ? (1 + damp) * g->cvac * g->dt * g->rdy : 0) * m->rmuz; \
    const float pz_muy =                                                  \
        ((nz > 1) ? (1 + damp) * g->cvac * g->dt * g->rdz : 0) * m->rmuy; \
    const float pz_mux =                                                  \
        ((nz > 1) ? (1 + damp) * g->cvac * g->dt * g->rdz : 0) * m->rmux; \
...
void vacuum_advance_e_pipeline_scalar(pipeline_args_t* args,
                                      int pipeline_rank,
                                      int n_pipeline);

void vacuum_advance_e_pipeline_v4(pipeline_args_t* args,
                                  int pipeline_rank,
                                  int n_pipeline);
