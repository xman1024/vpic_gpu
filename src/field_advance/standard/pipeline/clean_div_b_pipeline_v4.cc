...
void clean_div_b_pipeline_v4(pipeline_args_t* args,
                             int pipeline_rank,
                             int n_pipeline) {
...
#define NEXT_STENCIL(n) \
    ...
    for (; n_voxel > 3; n_voxel -= 4) {
        ...
        load_4x4_tr(&f00->cbx, &f01->cbx, &f02->cbx, &f03->cbx, f0_cbx, f0_cby,
                    f0_cbz, f0_div_b_err);

        fx_div_b_err = v4float(fx0->div_b_err, fx1->div_b_err, fx2->div_b_err,
                               fx3->div_b_err);

        fy_div_b_err = v4float(fy0->div_b_err, fy1->div_b_err, fy2->div_b_err,
                               fy3->div_b_err);

        fz_div_b_err = v4float(fz0->div_b_err, fz1->div_b_err, fz2->div_b_err,
                               fz3->div_b_err);
        ...
        store_4x4_tr(f0_cbx, f0_cby, f0_cbz, f0_div_b_err, &f00->cbx, &f01->cbx,
                     &f02->cbx, &f03->cbx);
    }
