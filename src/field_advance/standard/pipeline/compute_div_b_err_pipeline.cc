...
void compute_div_b_err_pipeline_scalar(pipeline_args_t* args,
                                       int pipeline_rank,
                                       int n_pipeline) {
...
#define LOAD_STENCIL()    \
    ...
    for (; n_voxel; n_voxel--) {
        f0->div_b_err = px * (fx->cbx - f0->cbx) + py * (fy->cby - f0->cby) +
                        pz * (fz->cbz - f0->cbz);
        f0++;
        fx++;
        fy++;
...
#define NEXT_STENCIL(n) \
    ...
    for (; n_voxel > 3; n_voxel -= 4) {
        ...
        f0_div_b_err = fma(vpx, fx_cbx - f0_cbx,
                           fma(vpy, fy_cby - f0_cby, vpz * (fz_cbz - f0_cbz)));

        store_4x1_tr(f0_div_b_err, &f00->div_b_err, &f01->div_b_err,
                     &f02->div_b_err, &f03->div_b_err);
    }
...
static void compute_div_b_err_pipeline_v8(pipeline_args_t* args,
                                          int pipeline_rank,
                                          int n_pipeline) {
    field_t* ALIGNED(128) f = args->f;
    const grid_t* g         = args->g;
    ...
    field_t *ALIGNED(16) f00, *ALIGNED(16) f01, *ALIGNED(16) f02,
        *ALIGNED(16) f03;  // Voxel quad
    field_t *ALIGNED(16) f04, *ALIGNED(16) f05, *ALIGNED(16) f06,
        *ALIGNED(16) f07;  // Voxel quad

    field_t *ALIGNED(16) fx0, *ALIGNED(16) fx1, *ALIGNED(16) fx2,
        *ALIGNED(16) fx3;  // Voxel quad +x neighbors
    field_t *ALIGNED(16) fx4, *ALIGNED(16) fx5, *ALIGNED(16) fx6,
        *ALIGNED(16) fx7;  // Voxel quad +x neighbors

    field_t *ALIGNED(16) fy0, *ALIGNED(16) fy1, *ALIGNED(16) fy2,
        *ALIGNED(16) fy3;  // Voxel quad +y neighbors
    field_t *ALIGNED(16) fy4, *ALIGNED(16) fy5, *ALIGNED(16) fy6,
        *ALIGNED(16) fy7;  // Voxel quad +y neighbors

    field_t *ALIGNED(16) fz0, *ALIGNED(16) fz1, *ALIGNED(16) fz2,
        *ALIGNED(16) fz3;  // Voxel quad +z neighbors
    field_t *ALIGNED(16) fz4, *ALIGNED(16) fz5, *ALIGNED(16) fz6,
        *ALIGNED(16) fz7;  // Voxel quad +z neighbors
...
#define NEXT_STENCIL(n) \
    ...
    for (; n_voxel > 3; n_voxel -= 8) {
        ...
        fx_cbx = v8float(fx0->cbx, fx1->cbx, fx2->cbx, fx3->cbx, fx4->cbx,
                         fx5->cbx, fx6->cbx, fx7->cbx);

        fy_cby = v8float(fy0->cby, fy1->cby, fy2->cby, fy3->cby, fy4->cby,
                         fy5->cby, fy6->cby, fy7->cby);

        fz_cbz = v8float(fz0->cbz, fz1->cbz, fz2->cbz, fz3->cbz, fz4->cbz,
                         fz5->cbz, fz6->cbz, fz7->cbz);

        f0_div_b_err = fma(vpx, fx_cbx - f0_cbx,
                           fma(vpy, fy_cby - f0_cby, vpz * (fz_cbz - f0_cbz)));

        store_8x1_tr(f0_div_b_err, &f00->div_b_err, &f01->div_b_err,
                     &f02->div_b_err, &f03->div_b_err, &f04->div_b_err,
                     &f05->div_b_err, &f06->div_b_err, &f07->div_b_err);
    }
