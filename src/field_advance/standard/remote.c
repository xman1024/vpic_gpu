...
void begin_remote_ghost_tang_b(field_t* ALIGNED(128) field, const grid_t* g) {
...
#define BEGIN_RECV(i, j, k, X, Y, Z)                                          \
    begin_recv_port(                                                          \
        i, j, k, (1 + n##Y * (n##Z + 1) + n##Z * (n##Y + 1)) * sizeof(float), \
        g)
...
void end_remote_ghost_tang_b(field_t* ALIGNED(128) field, const grid_t* g) {
    const int nx = g->nx, ny = g->ny, nz = g->nz;
    int face, x, y, z;
    float *p, lw, rw;
...
#define END_RECV(i, j, k, X, Y, Z)                                     \
    BEGIN_PRIMITIVE {                                                  \
        ...
        if (p) {                                                       \
            ...
            Z##Y##_EDGE_LOOP(face) field(x, y, z).cb##Y =              \
                rw * (*(p++)) + lw * field(x + i, y + j, z + k).cb##Y; \
            Y##Z##_EDGE_LOOP(face) field(x, y, z).cb##Z =              \
                rw * (*(p++)) + lw * field(x + i, y + j, z + k).cb##Z; \
        }                                                              \
    }                                                                  \
    END_PRIMITIVE
...
void begin_remote_ghost_norm_e(field_t* ALIGNED(128) field, const grid_t* g) {
    const int nx = g->nx, ny = g->ny, nz = g->nz;
    int size, face, x, y, z;
    float* p;
...
void end_remote_ghost_norm_e(field_t* ALIGNED(128) field, const grid_t* g) {
    const int nx = g->nx, ny = g->ny, nz = g->nz;
    int face, x, y, z;
    float *p, lw, rw;
...
#define END_RECV(i, j, k, X, Y, Z)                                    \
    BEGIN_PRIMITIVE {                                                 \
        ...
        if (p) {                                                      \
            ...
            lw   = (lw - g->d##X) / (lw + g->d##X);                   \
            face = (i + j + k) < 0 ? n##X + 1 : 0; /* Interpolate */  \
            X##_NODE_LOOP(face) field(x, y, z).e##X =                 \
                rw * (*(p++)) + lw * field(x + i, y + j, z + k).e##X; \
        }                                                             \
    }                                                                 \
    END_PRIMITIVE
    END_RECV((-1), 0, 0, x, y, z);
...
void begin_remote_ghost_div_b(field_t* ALIGNED(128) field, const grid_t* g) {
    const int nx = g->nx, ny = g->ny, nz = g->nz;
    int size, face, x, y, z;
    float* p;
...
void end_remote_ghost_div_b(field_t* ALIGNED(128) field, const grid_t* g) {
    const int nx = g->nx, ny = g->ny, nz = g->nz;
    int face, x, y, z;
    float *p, lw, rw;
...
#define END_RECV(i, j, k, X, Y, Z)                                         \
    BEGIN_PRIMITIVE {                                                      \
        ...
        if (p) {                                                           \
            ...
            lw   = (lw - g->d##X) / (lw + g->d##X);                        \
            face = (i + j + k) < 0 ? n##X + 1 : 0; /* Interpolate */       \
            X##_FACE_LOOP(face) field(x, y, z).div_b_err =                 \
                rw * (*(p++)) + lw * field(x + i, y + j, z + k).div_b_err; \
        }                                                                  \
    }                                                                      \
    END_PRIMITIVE
    END_RECV((-1), 0, 0, x, y, z);
...
#define BEGIN_RECV(i, j, k, X, Y, Z)                                    \
    begin_recv_port(                                                    \
        i, j, k,                                                        \
        (2 * n##Y * (n##Z + 1) + 2 * n##Z * (n##Y + 1) + n##Y * n##Z) * \
            sizeof(float),                                              \
        g)

#define BEGIN_SEND(i, j, k, X, Y, Z)                                           \
    BEGIN_PRIMITIVE {                                                          \
        size = (2 * n##Y * (n##Z + 1) + 2 * n##Z * (n##Y + 1) + n##Y * n##Z) * \
               sizeof(float);                                                  \
        p = (float*)size_send_port(i, j, k, size, g);                          \
        if (p) {                                                               \
...
#define BEGIN_RECV(i, j, k, X, Y, Z)                                          \
    begin_recv_port(                                                          \
        i, j, k, (n##Y * (n##Z + 1) + n##Z * (n##Y + 1) + 1) * sizeof(float), \
        g)

#define BEGIN_SEND(i, j, k, X, Y, Z)                                         \
    BEGIN_PRIMITIVE {                                                        \
        size = (n##Y * (n##Z + 1) + n##Z * (n##Y + 1) + 1) * sizeof(float);  \
        p    = (float*)size_send_port(i, j, k, size, g);                     \
        if (p) {                                                             \
            (*(p++))                       = g->d##X;                        \
            face                           = (i + j + k) < 0 ? 1 : n##X + 1; \
...
#define BEGIN_RECV(i, j, k, X, Y, Z) \
    begin_recv_port(i, j, k,         \
                    (1 + 2 * (n##Y + 1) * (n##Z + 1)) * sizeof(float), g)
