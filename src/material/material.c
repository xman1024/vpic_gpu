...
int num_material(const material_t* m_list) {
...
material_id get_material_id(const material_t* m) {
    return m ? m->id : -1;
}

material_t* append_material(material_t* m, material_t** m_list) {
    int id;
    if (!m || !m_list)
        ERROR(("Bad args"));
    if (m->next)
        ERROR(("Material \"%s\" already in a list", m->name));
    if (find_material_name(m->name, *m_list))
        ERROR(("There is already a material named \"%s\" in list", m->name));
    id = num_material(*m_list);
    if (id >= max_material)
        ERROR(
            ("Too many materials in list to append material \"%s\"", m->name));
    m->id   = (material_id)id;
    m->next = *m_list;
    *m_list = m;
    return m;
...
material_t* material(const char* name,
                     float epsx,
                     float epsy,
                     float epsz,
                     float mux,
