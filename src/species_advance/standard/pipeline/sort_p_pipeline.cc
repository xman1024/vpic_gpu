...
void sort_p_pipeline(species_t* sp) {
    ...
    int vh =
        VOXEL(sp->g->nx, sp->g->ny, sp->g->nz, sp->g->nx, sp->g->ny, sp->g->nz);
    ...
    if (n_subsort != 1) {
        ...
        for (subsort = 0; subsort < n_subsort; subsort++) {
            for (pipeline_rank = 0; pipeline_rank < n_pipeline;
                 pipeline_rank++) {
        ...
        // Results ended up in the wrong place as a result of the ugly hack
