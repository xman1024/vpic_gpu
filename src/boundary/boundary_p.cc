...
void boundary_p(particle_bc_t* RESTRICT pbc_list,
    ...
    do {
        ...
        for (face = 0; face < 6; face++)
            if (shared[face]) {
                ...
                pi_send[face] = (particle_injector_t*)(((char*)mp_send_buffer(
                                                           mp, f2b[face])) +
                                                       16);
                n_send[face]  = 0;
            }
    ...
    for (face = 0; face < 6; face++)
        if (shared[face]) {
            ...
            mp_size_recv_buffer(
                mp, f2b[face], 16 + n_recv[face] * sizeof(particle_injector_t));
    ...
    do {
        ...
        LIST_FOR_EACH(sp, sp_list) {
            ...
            // movers than available from MIN_NP
            nm = sp->nm + max_inj;
            if (nm > sp->max_nm) {
    ...
    do {
        ...
        do {
            ...
            if (face == 6)
                ...
                mp_end_recv(mp, f2b[face]);
                pi = (const particle_injector_t*)(((char*)mp_recv_buffer(
                                                      mp, f2b[face])) +
                                                  16);
                n  = n_recv[face];
            } else
