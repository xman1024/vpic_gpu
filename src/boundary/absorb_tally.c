...
int interact_absorb_tally(absorb_tally_t* RESTRICT at,
...
void checkpt_absorb_tally(const particle_bc_t* RESTRICT pbc) {
...
particle_bc_t* restore_absorb_tally(void) {
    absorb_tally_t* at;
    RESTORE(at);
...
void delete_absorb_tally(particle_bc_t* RESTRICT pbc) {
...
particle_bc_t* absorb_tally(/**/ species_t* RESTRICT sp_list,
    ...
    return new_particle_bc_internal(
        at, (particle_bc_func_t)interact_absorb_tally, delete_absorb_tally,
        (checkpt_func_t)checkpt_absorb_tally,
        (restore_func_t)restore_absorb_tally, NULL);
}
