...
int link_boundary(link_boundary_t* lb,
                  particle_t* r,
                  particle_mover_t* pm,
                  field_t* f,
                  accumulator_t* a,
    ...
    if (!fp) {
        ...
        if (fp)
            ERROR(
                ("File %s already exists (probably from a earlier run or "
                 "recovery) ... Please move it or change link filename",
                 fname));
        fp = fopen(fname, "w");
        if (!fp)
