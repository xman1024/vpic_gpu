...
int interact_maxwellian_reflux(maxwellian_reflux_t* RESTRICT mr,
    ...
    ratio =
        sqrtf(((1 + ratio) * (dispx * dispx + dispy * dispy + dispz * dispz)) /
...
void checkpt_maxwellian_reflux(const particle_bc_t* RESTRICT pbc) {
...
particle_bc_t* restore_maxwellian_reflux(void) {
...
void delete_maxwellian_reflux(particle_bc_t* RESTRICT pbc) {
    FREE(pbc->params);
...
particle_bc_t* maxwellian_reflux(species_t* RESTRICT sp_list,
    ...
    return new_particle_bc_internal(
        mr, (particle_bc_func_t)interact_maxwellian_reflux,
        delete_maxwellian_reflux, (checkpt_func_t)checkpt_maxwellian_reflux,
        (restore_func_t)restore_maxwellian_reflux, NULL);
}

/* FIXME: NOMINALLY, THIS INTERFACE SHOULD TAKE kT */
void set_reflux_temp(/**/ particle_bc_t* RESTRICT pbc,
                     const species_t* RESTRICT sp,
                     float ut_para,
                     float ut_perp) {
    if (!pbc || !sp || ut_para < 0 || ut_perp < 0)
        ERROR(("Bad args"));
