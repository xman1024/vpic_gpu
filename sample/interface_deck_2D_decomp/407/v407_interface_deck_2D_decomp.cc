...
//
// rm -rf rundata field ehydro Hhydro Hehydro particle fft movie poynting
// velocity restart
//
//========================================================================

// ASCII Logging of IO writes to disk.  Since wall clock elapsed time is
// written, and these require an MPI_Allreduce, don't include this macro inside
// code which only executes on a single processor, else deadlock will ensue.
...
#define DIAG_LOG(MSG)                                            \
    {                                                            \
        ...
        if (!(fptmp = fopen(fnametmp, "a")))                     \
            ERROR(("Cannot open file %s", fnametmp));            \
...
begin_globals {
    ...
    double inv_nppc;  // 1 / nppc in the high density region - used in density
                      // scaling
    ...
    double
        topology_x;  // domain topology needed to normalize Poynting diagnostic
    double topology_y;
    double topology_z;
...
begin_initialization {
    ...
    mp_barrier(grid->mp);  // Barrier to ensure we started okay.
    ...
    float mp_me = 1836;  // Proton to electron mass ratio - physical value is
                         // 1836. All ions' masses are scaled to this ratio
    ...
    int load_particles =
        1;  // Flag to turn off particle load for testing wave launch.
            //??????????????????????????????????????????????????????????????????
    double nppc =
        4000;  // for species I1 if qi_I1 > qi_I2; for species I2 if qi_I2 >
               // qi_I1 electron and ion number in each region are the same
    ...
    // NOTE: The following definitions make the assumption that Te = Ti =
    // constant everywhere at t=0 and that
    //       total pressure balance is established across the interface at t=0
    double n_i_I2 = 2.9197e24;  // ion density of D (1/cc)
    double n_i_I1 =
        n_i_I2 * (Z_I2 + 1.0) /
        (Z_I1 + 1.0);  // ion density of Al (assuming pressure balance)

    double rho0 =
        n_i_I1 * A_I1 * m_e * mp_me;  // ion mass density of Al in g/cc

    double n_e;  // Set n_e to be the electron density in region of highest e-
                 // density
    ...
    double delta = speed_of_light /
                   sqrt(4.0 * M_PI * n_e * elementary_charge2 / m_e);  // cm
...
//nx = nx/20;
  box_size_x /= 80000.0/4096.0;
    ...
    // work out stopping time in terms of ion sound speed crossing time across
    // box in x for Au

    double cs_crossing_time_ps =
        1e12 * box_size_x /
        sqrt(Z_I2 * t_e * k_boltz / (m_e * mp_me * A_I2));  // ps
    float t_stop = cs_crossing_time_ps * 100 *
                   wpe1ps;  // Runtime (in 1/wpe) equal to 100 Cs crossing times

    int fft_ex_interval =
        int(M_PI / (1.2 * dt));  // Num steps between writing Ex in fft slice
    //????????????????????????????????????????????????????????????????????????????????????
    int energies_interval = 1000;
    // int field_interval         = int( cs_crossing_time_ps * 0.001 * wpe1ps /
    // dt );         // Num. steps between saving field, hydro data
    int field_interval = 5000;  // Num. steps between saving field, hydro data
    restart_interval   = 4000;  // DEBUG - test launch

    int quota_check_interval = 20;
    int velocity_interval    = field_interval;  //  Num steps between writing
                                             //  poynting flux; not used in NIC
    ...
    double Ne = nppc * nx * ny * nz;  // Number of macro electrons in box in
                                      // region with highest e- density
    Ne         = trunc_granular(Ne,
                        nproc());  // Make Ne divisible by number of processors
    double Npe = Lx * Ly * Lz;  // Number of physical electrons in box in sim.
                                // units: density is 1, wpe = 1

    double qe_1, qe_2, qi_I1, qi_I2;

    // NOTE: The following definitions make the assumption that Te = Ti =
    // constant everywhere at t=0

    // NOTE: RECOGNIZE THAT THE SEMANTICS ARE DIFFERENT FOR qi_I1 VS. qi_I2
    // DEPENDING
    //       ON WHICH REGION HAS THE HIGHEST ELECTRON DENSITY! THE LOWER DENSITY
    //       REGION qi_I1 OR qi_I2 ONLY IS USED FOR DEFINING THE AVERAGE NPPC IN
    //       THE PARTICLE LOAD; THIS IS DONE BECAUSE WE ARE FORCING UNIFORM
    //       WEIGHTING OF PARTICLES IN THE COLLISION OPERATOR.
    ...
    if (n_i_I1 * Z_I1 > n_i_I2 * Z_I2) {  // region 1 has highest e- density
        ...
        qe_1 =
            -Npe / (Ne * Z_I1 *
                    ((Z_I2 + 1.0) /
                     (Z_I1 + 1.0)));  // Charge per macro electron in region 1 -
                                      // note: we load Z_I1 electrons per load.
        qi_I1 = -qe_1 * Z_I1;  // Charge per I1 macro ion =  Z_I1 * charge per
                               // macro electron
        qe_2 =
            qe_1;  // Charge per macro electron in region 2 - same as region 1
        qi_I2 = -qe_1 * Z_I2;  // Charge per I2 macro ion =  Z_I2 * charge per
                               // macro electron
  ...
  double qi_I1               = -qe_1;                        // Charge per H macro ion
    ...
    // In CGS, variance of tan theta = 2 pi e^4 n_e dt_coll loglambda / (m_ab^2
    // c^3)
    ...
    // Define log-lambda (we could compute this, but let's set it to fixed value
    // for now)
    double loglambda = 3;
    {
        // Compute nu_e from plasma parameters given.  Spitzer collisions
        // assumed. According to NRL plasma formuary, nu_e = nu_0^{e\e} (2
        // u_the^3) where nu_0^{e\e} = 4 pi e^4 n_e lambda / (m_e^2 v_e^3) We
        // need to compute n_0^{e\e} for v_e = u_the*c

        // First, compute n_e_cgs
        double n_e_cgs = n_e;

        // Then, compute nu_e, assuming that v_e = uthe*c
        double nu_e_cgs = 4 * M_PI * pow(4.8032e-10, 4) * n_e_cgs * loglambda *
                          pow(9.1094e-28, -2) * pow(uthe * 3e10, -3);
    ...
    int self_collisions_only =
        0;  // Flag for particles to scatter off same species only
    int tstep_coll =
        ion_sort_freq;  // How frequently to apply collision operator:
                        // N.B. Needs to be a multiple of ion_sort_freq
    double dt_coll = tstep_coll * dt;  // Units of wpe

    // cvar = variance of tan theta in collision operator for electron-electron
    // collisions
    //      = (8 pi e^4 n_e log-lambda) / (m_e^2 c^3)
    double cvar =
        2.0 * nu_e * pow(uthe, 3) * dt_coll;  // Coefficient for collision model
    double nppc_max =
        40 *
        nppc;  // Used internally in collision algorithm.  Should be sufficient?

    // Throw warning if collision interval is too large
    if (nu_e * 2 * dt_coll > 0.1) {
        sim_log(
            "*** Warning: scattering interval may be too large to sample "
            "electron scattering accurately.");
    ...
    // delta_x0                = 0.5 * 1.0e-4;                          /*
    // initial layer thickness in cm */
    delta_x0            = 0.05 * 1.0e-4; /* initial layer thickness in cm */
    half_box_size_in_xi = (0.5 * box_size_x) / delta_x0;

    // Proton ion self diffusion coefficient for pure hydrogen plasma at mass
    // density 1 g/cc at ion temperature 1 keV (used in initializing diffusion
    // velocities in layer)
    ...
    sim_log("* Time step, max time, nsteps =  " << dt << " " << t_stop << " "
                                                << int(t_stop / (dt)));
    sim_log("* wpe1ps =                       " << wpe1ps);
    sim_log("* Debye length, delta, cell size in x, z = "
            << debye << " " << delta << " " << cell_size_x << " "
            << cell_size_z);
    sim_log("* Lx, Ly, Lz =                   " << Lx << " " << Ly << " "
                                                << Lz);
    sim_log("* perturb_ampl =                 " << perturb_ampl);
    sim_log("* nx, ny, nz =                   " << nx << " " << ny << " "
                                                << nz);
...
#if 0
  if ( n_i_I1 * Z_I1 > n_i_I2 * Z_I2 ) {  // region 1 has highest e- density
    sim_log("* Charge/macro electron       =  "<<qe_1);
    sim_log("* Charge/macro ion =             "<<qi_I1);
  } else {
    sim_log("* Charge/macro electron       =  "<<qe_2);
    sim_log("* Charge/macro ion =             "<<qi_I2);
  }
#endif
    ...
    sim_log("* T_e, T_i, m_e, m_i_I1, m_i_I2:  " << t_e << " " << t_i << " "
                                                 << 1 << " " << mime_I1 << " "
                                                 << mime_I2);
    ...
    sim_log(
        "* load_particles:                " << (load_particles ? "Yes" : "No"));
    sim_log(
        "* do_collisions:                 " << (do_collisions ? "Yes" : "No"));
    sim_log("* self_collisions_only:          "
            << (self_collisions_only ? "Yes" : "No"));
    ...
    // BJA - add these to global array to eliminate magic numbers in collision
    // operator
    global->Z_e  = -1;
    global->Z_I1 = Z_I1;
    global->Z_I2 = Z_I2;
    ...
    electron = define_species("electron", -1, 2.0 * max_local_np, -1,
                              ele_sort_freq, 1);
    sim_log("- Creating I1 ion species.");
    ion_I1 = define_species("I1", Z_I1 / mime_I1, 2.0 * max_local_np, -1,
                            ion_sort_freq, 1);
    sim_log("- Creating I2 ion species.");
    ion_I2 = define_species("I2", Z_I2 / mime_I2, 2.0 * max_local_np, -1,
                            ion_sort_freq, 1);
    ...
    boundary_data->ut_perp[electron->id] = uthe;  // vth_e in perp direction
    boundary_data->ut_para[electron->id] = uthe;  // vth_e in para direction
    boundary_data->ut_perp[ion_I1->id]   = uthi_I1;
    boundary_data->ut_para[ion_I1->id]   = uthi_I1;
    boundary_data->ut_perp[ion_I2->id]   = uthi_I2;
    boundary_data->ut_para[ion_I2->id]   = uthi_I2;

    int maxwellian_reinjection =
        add_boundary(grid, maxwellian_reflux, boundary_data);
...
#define iv_region                                                            \
    (x < hx * iv_thick || x > Lx - hx * iv_thick) /* all boundaries are i.v. \
                                                   */
    set_region_material(iv_region, "impermeable_vacuum", "impermeable_vacuum");
    set_region_bc(iv_region, maxwellian_reinjection, maxwellian_reinjection,
    ...
    // Multiplier on thermal velocity to account for weakly relativistic
    // distribution (necessary because Kevin's load makes < p^2/2m > the same
    // for each particle, not < K.E. >, which is somewhat lower due to
    // relativistic kinematics. This is the multiplier one applies to uthe in
    // the sampler to force < K.E. > to be the same
    //
    // Note:  This is EMPIRICALLY obtained for the Molvig PRL problem and needs
    // to be
    ...
    if (load_particles) {
        ...
        // Fast load of particles--don't bother fixing artificial domain
        // correlations
        ...
        // Note: with the tanh load, we are not initializing in global pressure
        // balance
        ...
        // Note: chi is in units of (1/microns). simulation spatial values are
        // in units of skin depths.
        ...
        if (fabs(qi_I1) > fabs(qi_I2)) {
            sim_log(
                "nparticles = " << Ne / (topology_x * topology_y * topology_z));
            ...
            repeat(Ne / (topology_x * topology_y * topology_z)) {
                ...
                if (iv_region)
                    continue;  // Particle fell in iv_region.  Don't load.

                // phase of sinusoidal perturbation
                double zphase = 0;
                if (grid->nz > 1)
                    zphase = 2.0 * M_PI * (z / Lz);

                // xi represents position from center of box in microns (chi is
                // in units of 1/microns)

                xi = (x - (Lx / 2.0) -
                      perturb_ampl * cos(zphase));  // distance from center of
                                                    // box in skin depths
                xi *= delta * 1e4;  // distance from center of box in microns

                // Rejection method - use probability of loading an ion of
                // species 1 and Z_I1 electrons at (x,y,z)

                if (uniform_rand(0, 1) < an1 * tanh(chi * xi) + bn1) {
                    inject_particle(ion_I1, x, y, z, maxwellian_rand(uthi_I1),
                                    maxwellian_rand(uthi_I1),
                                    maxwellian_rand(uthi_I1), qi_I1, 0, 0);

                    repeat(Z_I1) {
                        // Leading order correction to Juttner distribution;
                        // also fixes uthe to account for weakly relativistic
                        // effects

                        do {
                            vx = maxwellian_rand(uthe *
                                                 uthe_relativistic_correction);
                            vy = maxwellian_rand(uthe *
                                                 uthe_relativistic_correction);
                            vz = maxwellian_rand(uthe *
                                                 uthe_relativistic_correction);
                            u2 = vx * vx + vy * vy + vz * vz;
                        } while (uniform_rand(0, 1) <
                                 u2 * u2 * one_over_eight_theta);
                        inject_particle(electron, x, y, z, vx, vy, vz, qe_1, 0,
                                        0);
                ...
                // Rejection method - use probability of loading an ion of
                // species 2 and Z_I2 electrons at (x,y,z)

                if (uniform_rand(0, 1) < an2 * tanh(chi * xi) + bn2) {
                    inject_particle(ion_I2, x, y, z, maxwellian_rand(uthi_I2),
                                    maxwellian_rand(uthi_I2),
                                    maxwellian_rand(uthi_I2), qi_I2, 0, 0);
                    ...
                    repeat(Z_I2) {
                        ...
                        do {
                            ...
                            u2 = vx * vx + vy * vy + vz * vz;
                                 ...
                                 u2 * u2 * one_over_eight_theta);
                        inject_particle(electron, x, y, z, vx, vy, vz, qe_2, 0,
                                        0);
...
#define I1_PROFILE_PROBABILITY(xi)                                             \
    (1.03642 *                                                                 \
     ((0.5184201288791436 - 0.9377618161236364 * xi +                          \
       0.574908984738253 * pow(xi, 2) - 0.1546771982667282 * pow(xi, 3) +      \
       0.048328624369960646 * pow(xi, 4) - 0.021607896487013795 * pow(xi, 5) + \
       0.003199315832322355 * pow(xi, 6)) /                                    \
          (1 - 1.0598281629210033 * xi + 0.4034701980854526 * pow(xi, 2) -     \
           0.16656733984429437 * pow(xi, 3) +                                  \
           0.07810662218354122 * pow(xi, 4) -                                  \
           0.014952297721801492 * pow(xi, 5) +                                 \
           0.003891946432949665 * pow(xi, 6)) +                                \
...
#define I2_PROFILE_PROBABILITY(xi)                                             \
    (fabs(qi_I2 / qi_I1) * 2.0 *                                               \
     (0.23118956282383712 - 0.04366718965766797 * xi -                         \
      0.09636585279725629 * pow(xi, 2) - 0.003080684826052242 * pow(xi, 3) +   \
      0.013994021713531602 * pow(xi, 4) + 0.0037279452459577454 * pow(xi, 5) + \
      0.00028706869350233273 * pow(xi, 6)) /                                   \
    ...
    // Pade fit - low-Z mass flux function (see Mathematica worksheet
    // "interfacex.nb" and notes from 6 Aug. 2015) This term is equal to:
    //   a11(xi) * ( d log x/dxi + (z(xi)-x(xi))(1 + (Ai/AI)*((ZI+1)/(Zi+1)) d
    //   (ai/zi)/dxi )
    // and to be turned into a speed, must be multiplied by:
    //   (mI/mi)(1/ZI^2) Dp (T/1 keV)^5/2 Ai^(-0.5) x0^-1
    // to get mass flux and, therefore, divided by rho c to get speed in
    // simulation units (vi/c); to get heavy ion speed, set vI = -vi (mi/mI)

#define I2_MASS_FLUX_FUNCTION(xi)                                              \
    (0.1542759226444911 + 0.03788536348380374 * xi +                           \
     0.049021909111038944 * pow(xi, 2) - 0.027055568725443978 * pow(xi, 3) -   \
     0.02666788955260103 * pow(xi, 4) - 0.0059218828537447215 * pow(xi, 5) -   \
     0.00040972771099082993 * pow(xi, 6)) /                                    \
        (1 + 1.5040184829931 * xi + 1.1005467498678096 * pow(xi, 2) +          \
         0.44355972952928274 * pow(xi, 3) + 0.10151805763024849 * pow(xi, 4) + \
         0.012694566906927567 * pow(xi, 5) +                                   \
         0.0006930803042010016 * pow(xi, 6))

    // Total mass density variation as a function of xi, normalized to rho0

#define NORMALIZED_MASS_DENSITY(xi)                                       \
    (0.7495077162996403 - 0.5186071721292596 * xi +                       \
     0.21278251708066112 * pow(xi, 2) - 0.0938868243001879 * pow(xi, 3) + \
     0.02887440490448864 * pow(xi, 4)) /                                  \
...
#define PREPARE_VELOCITY_SPACE_DATA(SUFF, NAME)                               \
    {                                                                         \
        ...
        for (int ip = 0; ip < sp->np; ip++) {                                 \
            ...
            /* Compute real particle position from relative coords and grid   \
             * data */                                                        \
            ...
            float invgamma =                                                  \
                1 / sqrt(1 + p->ux * p->ux + p->uy * p->uy + p->uz * p->uz);  \
            ...
            if (abs(ivx) < NVX && abs(ivz) < NVZ && PMASK)                    \
...
#define DUMP_VELOCITY_DATA(SUFF, LEN, HEADER_SIZE)                          \
    {                                                                       \
        ...
        if (status == fail)                                                 \
            ERROR(("Could not open file."));                                \
        fileIO_##SUFF.seek(                                                 \
            uint64_t(HEADER_SIZE + (step / global->velocity_interval - 1) * \
                                       LEN * sizeof(float)),                \
            SEEK_SET);                                                      \
  ...
  if ( 1 ) {
    ...
    // reason to not tag the dumps is if you want only one restart dump (the
    // most recent one) to be stored at a time to conserve on file space. Note:
    // A restart dump should be the _very_ _last_ dump made in a diagnostics
    ...
    // executable must be identical to that used to generate the restart dumps
    // or else the behavior may be unpredictable.
  ...
  if ( step && should_dump(restart) ) {
    ...
    if (step > 0 && should_dump(restart)) {
        ...
        static const char* restart_fbase[2] = {"restart/restart0",
                                               "restart/restart1"};
...
#if 0
  {
    double elapsed_time = mp_elapsed( grid->mp );
    ...
    if (step > 0 && global->quota_check_interval &&
        ...
        if (mp_elapsed(grid->mp) > global->quota_sec) {
            //  if ( mp_elapsed(grid->mp) > 23.5*3600.0 ) {
...
/* Takizuka and Abe model for 2-particle elastic scattering from JCP 25, 205
   (1977). I, J:    particle indices P1, P2:  particle array heads M1, M2:
   relative masses of particles var:     variance of collision operator */
...
#define SCATTER_PARTICLES(I, J, P1, P2, M1, M2, SQRT_VAR)                     \
    do {                                                                      \
        ...
        if (uperp > 0) { /* General case */                                   \
            dux = ((ux * uz * sin_theta * cos_phi) -                          \
                   uy * u * sin_theta * sin_phi) /                            \
                      uperp -                                                 \
                  ux * one_m_cos_theta;                                       \
            duy = ((uy * uz * sin_theta * cos_phi) +                          \
                   ux * u * sin_theta * sin_phi) /                            \
                      uperp -                                                 \
                  uy * one_m_cos_theta;                                       \
...
begin_particle_collisions {
    // Put collision model here since particle injection occurs after the
    // particle sort (collision models require sorted particle array).

    // Eventually this needs to be integrated into the VPIC source tree, but for
    // now leave this in the input deck.

    if (global->do_collisions && global->load_particles && step > 0 &&
        step % global->tstep_coll == 0) {
        species_t *s1, *s2;
        ...
        if (initted == 0) {
            ...
            if (cd[isp1].s == NULL)
                ERROR(("Could not find electron species."));
            cd[isp1].m = 1;
            ...
            if (cd[isp1].s == NULL)
                ERROR(("Could not find I1 species."));
            ...
            if (cd[isp1].s == NULL)
                ERROR(("Could not find I2 species."));
        ...
        for (isp1 = 0; isp1 < nsp; ++isp1) {
            ...
            for (icell = ((grid->nx + 2) * (grid->ny + 2) * (grid->nz + 2)) - 2;
                ...
                var = cd[isp1].cvar * (double)npar1 *
                      global->inv_nppc;  // multiplying by density as per Bill's
                                         // mods to get density-variation of
                                         // collision operator
                sqrt_var      = sqrt(var);
                sqrt_half_var = sqrt(var / 2);

                // Fischer-Yates shuffle on key array to randomize the order of
                // particles in a cell
                for (i = 0; i < npar1; ++i) {
                    int j;
                    j = uniform_rand(
                        0,
                        (double)i +
                            1);  // j is a random integer satisfying 0 <= j <= i
                    if (j != i)
                        karr1[i].i = karr1[j].i;
                ...
                if (npar1 > 4) {
                    for (i = 0; i < npar1 - 4; i += 2)
                        SCATTER_PARTICLES(karr1[i].i, karr1[i + 1].i, phead1,
                                          phead1, 1.0, 1.0, sqrt_var);
                    if (i == npar1 - 4) {
                        SCATTER_PARTICLES(karr1[i].i, karr1[i + 1].i, phead1,
                                          phead1, 1.0, 1.0, sqrt_var);
                        SCATTER_PARTICLES(karr1[i + 2].i, karr1[i + 3].i,
                                          phead1, phead1, 1.0, 1.0, sqrt_var);
                    } else {
                        SCATTER_PARTICLES(karr1[i].i, karr1[i + 1].i, phead1,
                                          phead1, 1.0, 1.0, sqrt_half_var);
                        SCATTER_PARTICLES(karr1[i].i, karr1[i + 2].i, phead1,
                                          phead1, 1.0, 1.0, sqrt_half_var);
                        SCATTER_PARTICLES(karr1[i + 1].i, karr1[i + 2].i,
                                          phead1, phead1, 1.0, 1.0,
                                          sqrt_half_var);
                    }
                }
            }
        ...
        if (global->self_collisions_only == 0) {
            for (isp1 = 1; isp1 < nsp; ++isp1) {
                ...
                for (isp2 = 0; isp2 < isp1; ++isp2) {
                    sim_log("Cross-Scatter of species " << isp1 << " and "
                                                        << isp2);
                    ...
                    for (icell = ((grid->nx + 2) * (grid->ny + 2) *
                        ...
                        // multiply by density as per Bill's mods to get
                        // density-variation collisions
                        double npar_min = (npar1 < npar2 ? npar1 : npar2);
                        var             = (global->cvar * pow(cd[isp1].z, 2) *
                               pow(cd[isp2].z, 2) / (rmass * rmass)) *
                              npar_min * global->inv_nppc;
                        sqrt_var = sqrt(var);

                        // Fischer-Yates shuffle on key array to randomize the
                        // order of particles in a cell
                        for (i = 0; i < npar1; ++i) {
                            int j;
                            j = uniform_rand(
                                0, (double)i + 1);  // j is a random integer
                                                    // satisfying 0 <= j <= i
                            if (j != i)
                                karr1[i].i = karr1[j].i;
                            karr1[j].i = s1->partition[icell] + i;
    ...
    do {                                                                      \
        if (NPAR1 > 0 && NPAR2 > 0) {                                         \
            ...
            for (i = 0; i < iimax; ++i) {                                     \
                ...
                for (j = 0; j < ii + 1; ++j) {                                \
                    ++index1;                                                 \
                    SCATTER_PARTICLES(K1[index1].i, K2[index2].i, P1, P2, M1, \
                                      M2, (SQRT_VAR) / sqrt(ii + 1));         \
                }                                                             \
            }                                                                 \
            for (; i < (NPAR2); ++i) {                                        \
                ++index2;                                                     \
                for (j = 0; j < ii; ++j) {                                    \
                    ...
                    SCATTER_PARTICLES(K1[index1].i, K2[index2].i, P1, P2, M1, \
                        ...
                        if (npar1 > npar2)
                            CROSS_COLLISION_LOOP(npar1, npar2, phead1, phead2,
                                                 karr1, karr2, m1, m2,
                                                 sqrt_var);
                        else
                            CROSS_COLLISION_LOOP(npar2, npar1, phead2, phead1,
                                                 karr2, karr1, m2, m1,
                                                 sqrt_var);
                    }
                }
