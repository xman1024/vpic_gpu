...
begin_initialization {
  ...
  double qi_I1               = -qe_1;                        // Charge per H macro ion
    ...
    // Ion particle array sizes can be made smaller than those for electrons -
    // this definition below uses the semantics above, that the ions in region
    // II have the highest charge.
    ...
    sim_log("num electron, ion macroparticles: " << max_local_np_e << " "
                                                 << max_local_np_i);
    species_t *electron = NULL, *ion_I1 = NULL, *ion_I2 = NULL;
    sim_log("- Creating electron species.");
    electron = define_species("electron", -1, 1, max_local_np_e, max_local_nm_e,
                              ele_sort_freq, 1);
    sim_log("- Creating I1 ion species.");
    ion_I1 = define_species("I1", Z_I1, mime_I1, max_local_np_i, max_local_nm_i,
                            ion_sort_freq, 1);
    sim_log("- Creating I2 ion species.");
    ion_I2 = define_species("I2", Z_I2, mime_I2, max_local_np_i, max_local_nm_i,
                            ion_sort_freq, 1);
        ...
        if (fabs(qi_I1) > fabs(qi_I2)) {
            ...
            repeat(Ne / (topology_x * topology_y * topology_z)) {
                ...
                if (uniform(rng(0), 0, 1) < an1 * tanh(chi * xi) + bn1) {
                    inject_particle(ion_I1, x, y, z, normal(rng(0), 0, uthi_I1),
                    ...
                    repeat(Z_I1) {
                        ...
                        do {
                            vx = normal(rng(0), 0,
                                        uthe * uthe_relativistic_correction);
                            vy = normal(rng(0), 0,
                                        uthe * uthe_relativistic_correction);
                            vz = normal(rng(0), 0,
                                        uthe * uthe_relativistic_correction);
                            u2 = vx * vx + vy * vy + vz * vz;
                        } while (uniform(rng(0), 0, 1) <
                                 u2 * u2 * one_over_eight_theta);
                        inject_particle(electron, x, y, z, vx, vy, vz,
                                        fabs(qe_1), 0, 0);
                ...
                if (uniform(rng(0), 0, 1) < an2 * tanh(chi * xi) + bn2) {
                    inject_particle(ion_I2, x, y, z, normal(rng(0), 0, uthi_I2),
                ...
                for (isp2 = 0; isp2 < isp1; ++isp2) {
                    ...
                    for (icell = ((grid->nx + 2) * (grid->ny + 2) *
                        ...
                        for (i = 0; i < npar1; ++i) {
                            ...
                            j = uniform(
                                rng(0), 0,
                                (double)i + 1);  // j is a random integer
                                                 // satisfying 0 <= j <= i
                            if (j != i)
                                karr1[i].i = karr1[j].i;
                            karr1[j].i = s1->partition[icell] + i;
