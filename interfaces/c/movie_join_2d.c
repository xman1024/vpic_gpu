...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3 && argc != 4) {
        fprintf(stderr,
                "Usage:  %s [num_processors] [filename]"
    ...
    if (argc == 4) {
        ...
        if (decomp < 0 || decomp > 2) {
            fprintf(stderr,
                    "Warning: unknown topology; assuming x decomposition.");
            decomp = 0;
        }
    }
    ...
    printf("Input, output record lengths: %d %d\n", irecord_length,
           orecord_length);
    if (irecord_length <= 0)
        ierror("Cannot proceed. Input record length <= 0");
    if (orecord_length <= 0)
        ierror("Cannot proceed. Output record length <= 0");
    ...
    while (!feof(fparr[0])) {
        ...
        foreach_file {
            ...
            for (ix = 0; ix < nx; ++ix)
                for (iy = 0; iy < ny; ++iy)
                    for (iz = 0; iz < nz; ++iz)
                        odata_vec[(ix + ixmin) * ny_total * nz_total +
                                  (iy + iymin) * nz_total + (iz + izmin)] =
