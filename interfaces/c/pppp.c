/*
   ...
   pro ppp, pdata
     ...
     particlestruct = {  x:0.0,  y:0.0,  z:0.0, $    ; particle spatial coords
                        ux:0.0, uy:0.0, uz:0.0, $    ; particle normalized
   momenta q:0.0 }                     ; particle charge
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 4) {
        freq = atoi(argv[3]);
        if (freq <= 0)
            ERROR("Bad particle dump interval specified.\n");
    }
    ...
    **************************************************************************/
    ...
    foreach_file {
        ...
        if (!(fparr[ifile] = fopen(fname, "rb")))
            ERROR("Cannot open particle data files.\n");
        ...
        for (ip = 0; ip < ah.dim; ip += PBUF_SIZE) {
            ...
            for (ibuf = 0; ibuf < n_buf; ++ibuf) {
                if ((ip + ibuf) % freq == 0) {
                    ...
                    /* Compute real particle position from relative coords and
                     * grid data */
                    /* Store data first in tmp particle struct po so we can
                     * apply filter */
            ...
            po_buf[obuf].q=po.q;
                        ...
                        obuf++;
                        if (obuf == PBUF_SIZE) {
                            fwrite(&po_buf, sizeof(struct opdata), PBUF_SIZE,
                                   ofp);
                            obuf = 0;
                        }
                    }
            ...
            if (obuf != 0)
                fwrite(&po_buf, sizeof(struct opdata), obuf, ofp);
        }
    ...
    printf(
        "Number of particles processed: %d\nNumber of particles written to "
        "file: %d\n",
