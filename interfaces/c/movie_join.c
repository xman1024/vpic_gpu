/*
   ...
   fe[0], fe[1], ... fe[ix*NVX + ivx] ... fe[(NX-1)*NVX + NVX-1]    <--frame 1
   data

   ...

   fe[0], fe[1], ... fe[ix*NVX + ivx] ... fe[(NX-1)*NVX + NVX-1]    <--frame NF
   data
...
int main(int argc, char* argv[]) {
    ...
    nproc = atoi(argv[1]);
    if (nproc <= 0)
        ERROR("Bad number of input files requested.\n");
    nx_output = atoi(argv[2]);
    if (nx_output <= 0)
        ERROR("Bad nx requested.\n");
    ...
    **************************************************************************/
    ...
    foreach_file {
        ...
        if (!(fparr[ifile] = fopen(fname, "rb"))) {
            printf(
                "Cannot open electron phase space files. Skipping ahead to H "
                "ions.\n");
    ...
    if (nx_total % nx_output)
        fprintf(stderr,
                "Warning:  requested number of x points doesn't "
                "evenly divide nx_total.\n");
    data_vec =
        (float*)emalloc((size_t)(nx_total * nvx_output) * sizeof(*data_vec));
    odata_vec =
        (float*)emalloc((size_t)(nx_output * nvx_output) * sizeof(*odata_vec));
    ...
    while (not_eof) {
        ...
        foreach_file {
            fread(&data_vec[ix], sizeof(float), nxarr[ifile] * nvx_output,
                  fparr[ifile]);
        ...
        for (ix = 0; ix < nx_output; ++ix) {
            ...
            for (ivx = 0; ivx < nvx_output; ++ivx) {
                ...
                for (ixa = 0; ixa < nx_ave && ix * nx_ave + ixa < nx_total;
                     ++ixa)
                    odata_vec[ix * nvx_output + ivx] +=
                        data_vec[(ix * nx_ave + ixa) * nvx_output + ivx];
                /* divide by nx_ave unless interval straddles nx_total; then,
                   divide by actual number of cells summed. */
                odata_vec[ix * nvx_output + ivx] /= ixa;
            }
        }

        /* Write odata_vec into output stream */
        WRITE_VEC(ofp, odata_vec, nx_output * nvx_output, sizeof(float),
                  write_binary);
        printf("done.\n");
    }
    fclose(ofp);
...
Process_Hydrogen_ions:
    ...
    foreach_file {
        ...
        if (!(fparr[ifile] = fopen(fname, "rb"))) {
            printf(
                "Cannot open H phase space files. Skipping ahead to He "
                "ions.\n");
...
Process_Helium_ions:
    ...
    foreach_file {
        ...
        if (!(fparr[ifile] = fopen(fname, "rb"))) {
            printf(
                "Cannot open He phase space files. Skipping ahead to "
                "fields.\n");
            goto Process_fields;
        }
        /* First element in each file is the num of cells in local x mesh */
...
Process_fields:
    ...
    ************************************************************************/
    if (!(ologp = fopen("movie_log.dat", "w")))
